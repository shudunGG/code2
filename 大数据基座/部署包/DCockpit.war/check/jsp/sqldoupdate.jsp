...
<%!List<Map<String, String>> datelist = new ArrayList<Map<String, String>>();
	...
	String delimiterStr = isOracle ? "/* GO */" : "GO";

	private String initPara(String path, String method) {
		String msg = "";
		if (method.toLowerCase().equals("post") && path != null && path != "") {
			try {
				executeSqlFile(new File(path));
			} catch (Exception e) {
...
<%
	try {
		...
		if (request.getParameter("path") != null && request.getParameter("path") != "") {
			String error = initPara(request.getParameter("path"), request.getMethod());
