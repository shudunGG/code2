...
(function(win, $) {
	...
	var renderTree = function(data, acc) {
		...
		// 	node.rowkey = pRowkey + ROW_KEY_SEP + i + '';

		// 	// 若未配置节点图标，采用默认图标
		// 	if(!node.icon) {
		// 		node.icon = 'node-icon-default';
		// 	}

		// 	// 缓存节点
		// 	CacheMgr.cacheNodeData(node);
		// 	// 缓存expanded为true的非leaf节点，方便后续处理
		// 	CacheMgr.cacheDefaultExpandedNodes(node);

		// 	// 节点的缩进
		// 	node.indent = (node.rowkey.split(ROW_KEY_SEP).length - 2 + (node.isLeaf ? 1 : 0)) * 20;

		// 	var items = node.items;

		// 	if(items && items.length) {
		// 		node.subNodes = renderTree(items, node.rowkey);
		// 	}

		// 	html.push(M.render(treeNodeTempl, node));
		// });
	...
	// 			code: node.code
	// 		},
	// 		url: Util.getRightUrl(LeftAccTree.loadUrl + 'lazyData.json'),
	// 		context: node,
	// 		success: function(data) {
	// 			var html,
	// 				$node,
	...
	// 			if(data && data.length) {
	// 				html = M.render(subNodesTempl, {
	// 					subNodes: renderTree(data, rowkey)
	// 				});

	// 				// 父节点
	// 				$node = $con.find('[data-rowkey="' + rowkey + '"]');
	...
	// 点击节点行
	// }).on('click', '.node-line', function(event) {
	// 	var $el = $(this),
	// 		$node = $el.parent(),
	...
	// // 点击展开按钮
	// }).on('click', '.node-toggle', function(event) {
	...
	// 		node = CacheMgr.getNodeData(rowkey),
	// 		cfg = CacheMgr.getNodeData(rowkey.substring(0, 1));

	// 		// 若配置为懒加载 并且 还未加载，则动态获取数据
	// 		if(cfg.isLazy && !$node.find('> .sub-nodes').length) {
	// 			fetchNodes(node);
