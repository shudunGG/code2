...
function EMsgDialog(currUserId) {
    ...
    var initEmotion = function () {
        Util.loadJs('fui/js/lib/jquery.emotion.js', function () {
            $('.emsg-emotion-icon').emotion({
                target: '.emsg-message-text',
                path: '../../emsg/images/emotion/'
            });
        });
    };
    ...
    var initUploader = function () {
        ...
        var uploader = WebUploader.create({
            dnd: '#EmsgText',
            auto: true,
            swf: Util.getRightUrl('fui/js/lib/webuploader/Uploader.swf'),
                ...
                Util.ajax({
                    ...
                    success: function (data) {
                        try {
                            if (data.success) {
                                $currName.html(n);
                                $this.data('origname', n);
                                sessionList[currSession.sessionId].name = n;
                                var $li = getSessionNav(currSession.uid);
                                $('.emsg-user-name', $li).html(n);
                                epoint.showTips('修改成功', {
                                    state: 'success'
                                });
                            } else {
                                epoint.showTips('修改讨论组名称失败', {
                                    state: 'error'
                                });
                            }
                        } catch (error) {}
    ...
    var activeSession = function (sessionId, isNewSession) {
        ...
        $currImg.attr('src', EmsgConfig.userImg).attr('src', s.imgUrl).off('error').on('error', function () {
    ...
    this.init = function () {
        ...
        if (window.WebUploader) {
            ...
            Util.loadJs('fui/js/lib/webuploader/webuploader.min.js', function () {
                initUploader();
            });
    ...
    this.show = function (pos) {
        ...
        if (!pos || pos === 'max') {
            ...
            if (t < 0) t = 0;
            if (l < 0) l = 0;
            $eMsgDialog.removeClass('bottom min').css({
                'left': l,
                'top': t,
                'width': dw,
                'height': dh,
