...
/* global _rootPath, _v, _t, epctrl, Util*/
...
(function (win, $) {
    ...
    var getPathByName = function (name) {
        ...
        var url = window._rootPath ? _rootPath + '/fui/js/widgets/epcontrols/' + name + '/' + name + '.js' : '';

        // 加上框架js的版本控制
        if (window._v && window._t) {
            url += '?_t=' + _v + _t;
        }
        return url;
    };
    ...
    Control.prototype = {
        ...
        _getControlPath: function () {
            return 'fui/js/widgets/epcontrols/' + this.type.toLowerCase() + '/';
        },
        // 事件实现
        /**
         * 绑定事件
        ...
        _loadJsPromise: function (url) {
            ...
            script.src = window._rootPath ? _rootPath + '/' + url : url;
            // append to head
            document.getElementsByTagName('head')[0].appendChild(script);
        ...
        loadCss: function (urls, target, pos) {
            ...
            for (var i = 0, l = urls.length; i < l; ++i) {
                ...
                if (document.getElementById('style-' + urls[i])) {
                ...
                link.rel = 'stylesheet';
                // 改用路径做id 避免重复加载
                link.id = 'style-' + urls[i];

                // link.href = (Util && Util.getRightUrl) ? Util.getRightUrl(urls[i]) : urls[i];
                link.href = window._rootPath ? _rootPath + '/' + urls[i] : urls[i];
        ...
        loadHtml: function (url) {
            ...
            return $.ajax({
                url: (window._rootPath ? _rootPath + '/' + url : url),
                type: 'GET',
                async: true,
                dataType: 'HTML'
            }).done(function (data) {
        ...
        addSkin: function () {
            try {
                this.addCss(this._getControlPath() + 'themes/' +
                    Util.getThemeName() + '/skins/' +
                    Util.getSkinName() + '/skin.css');
            } catch (err) {}
        },
        // 为控件设置js资源
        setJs: function (url) {
    ...
    // 名称兼容处理
    win.epctrl.overWrite = win.epctrl.overwrite;
    win.epointControl = win.epctrl;
