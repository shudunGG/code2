 ...
 * @LastEditors: chends
 * @LastEditTime: 2018-07-19 16:01:42
 * @Description: 分类搜索组件
 */
(function (win, $) {
    ...
    var CLASSIFICATION_TPL = '<li class="classify-search-classify-item {{#isActive}}active{{/isActive}}" data-id="{{id}}"><span class="classify-search-classify-icon {{iconCls}}"></span>{{name}}包含<span class="classify-search-classify-kw"></span><span class="classify-search-classify-target">{{target}}</span></li>';

    var HISTORY_TPL = '<li class="classify-search-history-item" title="{{name}}"><span class="classify-search-history-text">{{name}}</span><span class="action-icon icon-removethin classify-search-history-remove"></span></li>';
    ...
    $.extend(ClassifySearch.prototype, {
        init: function () {
            if (this.opt.autoLoadStyle && !ClassifySearch._styleLoaded) {
                Util.loadCss('fui/js/widgets/classifysearch/classifysearch.css', '#common-skin', 'Before', true);
        ...
        _initView: function () {
            ...
            this.$historyPanel = $('<div class="classify-search-history-panel"><ul class="classify-search-history-list"></ul><div class="classify-search-history-footer"><span class="classify-search-history-footer-icon"></span><span class="classify-search-history-footer-text">清空历史记录</span></div></div>');
            this.$historyList = $('.classify-search-history-list', this.$historyPanel);

            this.$classifyPanel = $('<div class="classify-search-classify-panel"><ul class="classify-search-classify-list"></ul></div>');
            ...
            $.each(this.opt.category, function (i, item) {
                ...
                html += Mustache.render(CLASSIFICATION_TPL, item);
            });
        ...
        _initEvent: function () {
            ...
            this.$container
                ...
                .on('click', '.classify-search-history-text', function () {
                    ...
                    that.$input.val($this[0].title).trigger('focus');
                })
                // 历史记录删除
                ...
                .on('click', '.classify-search-history-footer', function () {
                    ...
                    // that.$container.trigger('histortChange');
                })
                // 分类项目点击
            ...
            var classifyCodeActions = {
                '13': function (k, v) {
                    ...
                    this.$classifyList.children(':eq(0)').addClass('active').siblings().removeClass('active');
                    this.opt.enter.call(that, k, v);
                    this.hidePopup();
                    this.$input.blur().val('');

                    // rerender 
                    this.$container.trigger('histortChange');
            ...
            this.$input.on('keyup', function (e) {
                ...
                if (!key) {
                    key = that.$classifyList.children(':eq(0)').data('id');
                }

                console.log(code, key, val);

                if (val) {
                    that._fillKeyWord(val);
        ...
        _itemMove: function (direction) {
            var $items = $(this.$container.hasClass('classify') ? this.$classifyList : this.$historyList).children();
