...
(function (win, $) {
    ...
    $.extend(Util, {
        ...
        openSelfWin: function (url) {
            Util.getSafeLocation().setHref(url);
        },
        ...
        loadPageModule: function (path) {
            $.ajax({
                type: 'POST',
                dataType: 'html',
                url: win.SrcBoot ? SrcBoot.handleResPath(path.templ) : Util.getRightUrl(path.templ),
                data: {},
                ...
                401: function (xhr) {
                    ...
                    if (top.window.quickLogin !== undefined) {
                        ...
                        top.Util.loadPageModule({
                            templ: 'fui/pages/quicklogin/quicklogin.tpl',
                            css: 'fui/pages/quicklogin/quicklogin.min.css',
                            js: 'fui/pages/quicklogin/quicklogin.js',
                            callback: function () {
                                top.quickLogin.show();
                            }
                        });
        ...
        ajax: function (options) {
            ...
            var _ajax = _jqXhr.then(function (data) {
                ...
                if ((typeof data !== 'object') && options.dataType.toLowerCase() === 'json') {
        ...
        getThemeName: function () {
            return $.cookie('_theme_');
        },

        getSkinName: function () {
            var themeName = this.getThemeName();

            return $.cookie('_' + themeName + '_skin_') || 'default';
        },
...
(function (win, $) {
    ...
    var initEvent = function () {
        $left.on('click', '.fui-left-toggle', function (e) {
...
(function (win, $) {
    ...
    var parse = function () {
        var $inners = $forms.find('[role="form"]'),
            $rows = $forms.find('[role="row"]');
...
(function (win, $) {
    ...
    var _adjustHeight = function () {
        ...
        $content.height(win_h - toolbar_h - condition_h - notice_h - toolbarbottom_h);
...
(function (win, $) {
    ...
    if (showAsTooltip) {
        ...
        // $helper.addClass('fui-toolbar-helper');

        // 为了方便ie8将字体图标替换成png图标，帮助按钮的图标换成背景图片了
        // $helper.prepend('<i class="icon-askcircle"></i>');
...
(function (win, $) {
    ...
    if (!opened) {
        ...
        var insertInput = function () {
            ...
            // 给两个控件绑定事件
            toolbarSearch.on('valuechanged', function (e) {
    ...
    */
    ...
    !opened && $trigger.on('click', function (e) {
        ...
        // tag close 则打开 否则关闭
        // if (('searcherclose').indexOf(status) != -1) {
        //     switchStatus('open');
        // } else {
        //     switchStatus('close');
        // }
    ...
    var _getControls = function (params) {
    ...
    var _resetValue = function () {
        ...
        for (var i = 0, l = arr.length; i < l; ++i) {
            ...
            if (arr[i].id == primaryId) {
                toolbarSearch && toolbarSearch.setText('');
            }
        }
    };
    ...
    var _renderTag = function (obj) {
        ...
        item.push('<li class="fui-search-item l"><span class="fui-search-item-text l" title="');
        item.push(obj.text + '">' + obj.text.replace(/>/g, '&gt;').replace(/</g, '&lt;'));
        item.push('</span><span class="fui-search-item-remove l" title="移除" ');
        // checkboxlist 会传递单个的value过来
        if (obj.value) {
            item.push('data-value="' + obj.value + '" ');
        }
        item.push('data-id="' + obj.id + '"></span></li>');

        return item.join('');
    };
    // 用于存储标签名 这两个变量相关的是支持显示为 label：vale 的形式 暂时不用了
    // var TAGNAMES = {},
    //     showTagName = $condition.attr('showtagname') == 'true' ? true : false;
    ...
    var renderTags = function () {
        ...
        for (var i = 0, l = controls.length; i < l; i++) {
            ...
            // if (showTagName) {
            //     if (!TAGNAMES[control.id]) {
            //         TAGNAMES[control.id] = $(control.el).parent().prev().text()
            //     }
            // }
            ...
            if (control.type == 'checkboxlist') {
                ...
                for (var j = 0, len = values.length; j < len; ++j) {
                    html.push(_renderTag({
                        // text: showTagName ? (TAGNAMES[control.id] + '：' + text) : text,
                        value: values[j],
                        text: text[j],
                        id: control.id
                    }));
...
//     var showNav = $accsWrap.attr('showNav') === "true";
...
//         return '<li class="acc-menu-item" data-ref="' + id + '">' + title + '</li>';
//     };
...
//         html.push('<i class="acc-menu-trigger"></i>');
...
//             if (showNav) {
//                 var id = acc.id ? acc.id : (acc.id = 'accordion_' + i);
...
//                 if ($target.hasClass('acc-menu-trigger')) {
//                     $accNav.toggleClass('active');
//                 } else if ($target.hasClass('acc-menu-item')) {
//                     id = $target.data("ref");
//                     $accItem = $('#' + id);

//                     scrollEl = Util.getFirstScrollEl($accItem[0]);

//                     if (scrollEl) {
//                         $scrollEl = $(scrollEl);
//                         scrollTop = $accItem.offset().top + $scrollEl.scrollTop() - $scrollEl.offset().top;
...
(function (win, $) {
    ...
    var showNav = $accsWrap.attr('showNav') === "true" && $fuiContent.length > 0;
    ...
    $fuiContent.on('scroll', function (event) {
        ...
        timer = setTimeout(function () {
            $items.each(function (i, el) {
                ...
                if (top >= 0) {
                    $(".acc-menu-item").eq(i).addClass('active')
...
(function (win, $) {
    ...
    var getPathPrefix = function (name) {
        var path = 'fui/pages/navpages'; // 默认路径
        if (srcdir) {
            path = srcdir; // 个性化路径
        }
    ...
    var initMaster = function (name) {
        ...
        $content.load(Util.getRightUrl(prefix + '_snippet.html'), function () {
            ...
            // 为了导航模板样式能够被皮肤覆写，需将其放置到皮肤css之前
            Util.loadCss(prefix + '.css', document.getElementById('common-skin'), 'Before', true);
...
// 用于页面上js配置项的分离：fe给前端，be给后端
// 不过目前后端集成是copy原型html，改为xhtml页面，以下代码应该不需要了
// (function(win, $) {
//     if (window._settings) {
...
(function (win, $) {
    ...
    $.ajaxSetup({
        beforeSend: function (XMLHttpRequest) {
            var csrfcookie = $.cookie(win.CSRF_COOKIE_NAME || '_CSRFCOOKIE');
            if (csrfcookie) {
...
(function () {
    ...
    // 加载文件自动提醒
    Util.loadJs('fui/js/lib/browsertips/browsertips.js');
}());

// 皮肤切换
(function (win, $) {

    var theme = $.cookie('_theme_') || 'grace';

    var skinSwitcher = {
        _$commonSkin: $('#common-skin'),
        _$miniSkin: $('#miniui-skin'),
        // _$themeSkin: $('#theme-skin'),
        updateSkin: function (skin) {
            $.cookie('_' + theme + '_skin_', skin, {
                path: '/',
                expires: 365
            });
            var path = [
                'fui/css/themes/' + theme + '/skins/' + skin + '/skin.css',
                'fui/js/miniui/themes/' + theme + '/skins/' + skin + '/skin.css'
                // 'fui/pages/themes/' + theme + '/skins/' + skin + '/skin.css',
            ];
            var d = +new Date();

            this._$commonSkin.length && this._$commonSkin.attr('href', Util.getRightUrl(path[0] + '?t=' + d));
            this._$miniSkin.length && this._$miniSkin.attr('href', Util.getRightUrl(path[1] + '?t=' + d));
