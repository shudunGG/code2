...
mini.extend(mini.OAUploader, mini.UserControl, {
    ...
    uiCls: 'uc-oauploader',
    // 模板的地址，路径默认从webapp开始
    tplUrl: 'fui/js/epoint/uc/oauploader/oauploader.tpl',
    // css文件资源路径
    cssUrl: 'fui/js/epoint/uc/oauploader/oauploader.css',
    // 自定义控件对应后台控件模型类的全名（包名+类名）
    mapClass: 'com.epoint.basic.faces.fileupload.WebUploader',

    uploadUrl: '',
    // 允许的文件后缀，不带点，多个用逗号分割
    limitType: window.EpFrameSysParams ? EpFrameSysParams['fileLimitType'] : '',
    mimeTypes: '',
    auto: true,
    fileNumLimit: undefined,
    fileSizeLimit: window.EpFrameSysParams ? EpFrameSysParams['fileSizeLimit'] : undefined,
    ...
    _editSupportExt: '',
    // 预览地址
    _previewUrl: window.EpFrameSysParams ? EpFrameSysParams['uploadPreviewUrl'] : '',
    _previewExt: window.EpFrameSysParams ? EpFrameSysParams['uploadPreviewText'] : '',
    // 展示为文件列表
    ...
    set: function (kv) {
        ...
        this._props = $.extend(this._props, {
