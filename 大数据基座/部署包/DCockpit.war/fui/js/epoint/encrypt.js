...
(function (win, $) {
    ...
    function needEncrypt(url) {
        // 未开启 则不用处理
        if (!Util.getFrameSysParam('enableEncodeParam')) {
            return false;
        }
        if (!_skipEncodeUrlArr) {
            _skipEncodeUrlArr = Util.getFrameSysParam('skipEncodeUrl')
                ? Util.getFrameSysParam('skipEncodeUrl').replace(/^;+|;+$/g, '').split(';')
                : [];
        }
        // 相对路径 【./ 或者 ../】 肯定要加密
        if (/^(?:\.\/|\.\.\/)/.test(url)) {
        ...
        if (/^(http|https|ftp|data|ws|wss)/.test(url)) {
            ...
            // 三个组 协议 、 域名 、端口号
            var loc = win.Util && win.Util.getSafeLocation ? Util.getSafeLocation() : win.location;
            var c = loc.protocol + loc.host;
            var p1 = matchArr[1] || '',
    ...
    $.extend(win.Util, {
        ...
        _getUrlParams: function (url, prop) {
            if (!url) {
                url = Util.getSafeLocation().search;
            } else {
            ...
            if (params[Util.URL_ENCRYPT_PARAM_NAME]) {
                ...
                decryptStr = win.decodeURIComponent(decryptStr);
                // 将加密参数解密 并合并到 普通参数中
        ...
        getUrlParams: function (prop) {
            return Util._getUrlParams(Util.getSafeLocation().search, prop);
        },
        /**
         * 对 ajax 请求的参数加密
         *
         ...
         */
        addUrlParams: function(url, params, mode) {
            ...
            var paramsStr = '';
            // 如果为加密 则需要重新加密
            if (needEncrypt(url) || isEncrypted) {
                var obj = {};
