...
window.epoint = (function(epoint) {
    ...
    var controlResources = {
        ...
        combobox: {
            templ: '<input class="mini-combobox" id="{{fieldName}}" action="{{action}}" data="{{dataData}}" bind="{{bind}}"{{#textField}} textField="{{textField}}"{{/textField}}{{#valueField}} valueField="{{valueField}}"{{/valueField}}{{#emptyText}} emptyText="{{emptyText}}"{{/emptyText}} {{#required}}required="true"{{#requiredErrorText}} requiredErrorText="{{requiredErrorText}}"{{/requiredErrorText}}{{/required}}/>'
        },
        treeselect: {
        ...
        checkbox: {
            templ: '<div id="{{fieldName}}" class="mini-checkbox" bind="{{bind}}" {{#trueValue}}trueValue="{{trueValue}}"{{/trueValue}} {{#falseValue}}falseValue="{{falseValue}}"{{/falseValue}}></div>'
        },
        checkboxlist: {
            templ: '<div id="{{fieldName}}" class="mini-checkboxlist" action="{{action}}" data="{{dataData}}" bind="{{bind}}"{{#textField}} textField="{{textField}}"{{/textField}}{{#valueField}} valueField="{{valueField}}"{{/valueField}}></div>'
        },
        radiobuttonlist: {
            templ: '<div id="{{fieldName}}" class="mini-radiobuttonlist" action="{{action}}" data="{{dataData}}" bind="{{bind}}"{{#textField}} textField="{{textField}}"{{/textField}}{{#valueField}} valueField="{{valueField}}"{{/valueField}}></div>'
        },
        webuploader: {
        ...
        outputtext: {
            templ: '<div id="{{fieldName}}" class="mini-outputtext" bind="{{bind}}"{{#dataOptions}} data-options="{{dataOptions}}"{{/dataOptions}}></div>'
        }
    };
