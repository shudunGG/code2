...
$.extend(window.Util, {
    ...
    /**
     * 获取url参数
     * @param {String} name 需要获取的字段名，不传则返回所有参数
     */
    ...
    getData: function(options, successFn, extraParams) {
        ...
        if (Config.mockModules.indexOf(module) > -1) {
            preUrl = '';
        }

        if (Util[module + 'Timer']) {
            clearTimeout(Util[module + 'Timer']);
        }
        ...
        options.url = (isMock ? '' : preUrl) + Config.pageInterface[module];
        $.ajax(options).done(function(data) {
            successFn && successFn(data.custom, extraParams);
            // 如果定义了定时请求时间，则定时请求数据
            if (Config.moduleRefresh[module]) {
                Util[module + 'Timer'] = setTimeout(function() {
    ...
    // 打开弹窗
    openLayer: function(areaSize, href) {
        top.layer.open({
            type: 2,
            title: '',
            skin: 'layer-reset',
