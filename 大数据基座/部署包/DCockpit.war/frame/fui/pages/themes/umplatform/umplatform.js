...
var SCREEN_PAGE_TPL = '<a href="javascript:void(0);" data-page="{{page}}" class="page-item l mr10"><span>{{pageNum}}</span></a>',
    // 分屏模板
    SCREEN_TPL = '<div class="imac-screen l" data-title="第{{page}}分屏"></div>',
    // 桌面应用模板
    SCREEN_APP_TPL = '<div class="app-item trans" title="{{name}}" style="top:{{top}}px;left:{{left}}px" data-id="{{id}}"><span class="unread {{^count}}hidden{{/count}}">{{count}}</span><img src="{{icon}}" alt="" class="app-icon"><p class="app-name">{{name}}</p></div>',
    // 底部任务栏：任务模板
    TASK_ITEM_TPL = '<a href="javascript:void(0);" id="{{id}}-task" data-id="{{id}}" class="task-item l clearfix" title="{{name}}"><p class="task-name l">{{name}}</p></a>',
    // 下拉任务中的任务模板
    DROP_ITEM_TPL = '<li id="{{id}}-drop" data-id="{{id}}" class="drop-item" title="{{name}}">{{name}}</li>',
    // 工具栏：快捷应用模板
    ...
    // 搜索中的应用模板
    SEARCH_APPS_TPL = '<div class="app-item trans in-panel l" title="{{name}}" id="{{id}}-search" data-id="{{id}}"><img src="{{icon}}" alt="" class="app-icon"><p class="app-name">{{name}}</p></div>',
    // 我的看板按钮模板
    BOARD_BTN_TPL = '<div class="right-board-btn" data-id="{{id}}" data-url="{{url}}">{{name}}</div>',
    // 我的看板内容模板
...
(function (win, $) {
    ...
    $.extend(Util, {
        ...
        fixPath: function (data) {
            ...
            if (!/^data:image/.test(data.icon)) {
                ...
                // copy.icon = Util.getRightUrl('./images/app/' + data.icon);
    ...
    // // 重写Util.ajax方法，用于无界面标识时的提醒
    // Util._ajax = Util.ajax;
    // Util.ajax = function (opt) {

    //     // opt 中的succees 比then先触发 因此先去掉
    ...
    //     return Util._ajax(opt).then(function (data) {
    //         // 如果数据中存在特定的标识，表示非法访问。
    //         if (data.error) {
    //             // 此时表示失败 给出用户提醒

    //             // 并打断ajax的链式调用，返回一个失败的ajax
    //             return $.Deferred().reject('xs').promise();
    //         }
    //         // 否则 正常调用原success
...
// 加载imac背景图片
(function (win, $) {
    var DEFAULT_BG = 'frame/fui/pages/themes/umplatform/images/skinbg/bg.jpg';
    ...
    Util.ajax({
        ...
        if (data && data.bg) {
            ...
            localStorage.setItem('_umplatform_bg_', data.bg);
        } else {
            $bg[0].src = Util.getRightUrl(DEFAULT_BG);
            console && console.error('未返回背景，加载默认背景！');
            localStorage.setItem('_umplatform_bg_', DEFAULT_BG);
        }
    });
...
(function (win) {
    ...
    win.ImacEvent = new Util.UserEvent();
...
(function (win, $) {
    ...
    var appNumPerCol = ($(win).height() - OCCUPIED_H - APP_INIT_TOP) / (APP_HEIGHT + APP_GAP) >> 0;
    // app context menu
    var appCm;
    ...
    var renderAppsByPage = function (page, data) {
        ...
        $.each(data, function (i, item) {
            ...
            // 记录需要获取消息数目的应用集合
            if (item.msgCountUrl) {
                AppMgr.msgRemindApps.push({
                    id: item.id,
                    url: item.msgCountUrl
                });
            }
                ...
                if (data.status == 'success') {
                    ...
                    if (appData && appData.msgCountUrl) {
                        for (var i = 0, l = AppMgr.msgRemindApps.length; i < l; ++i) {
                            var currApp = AppMgr.msgRemindApps[i];
                            if (currApp.id == id) {
                                currApp._$count = null;
    ...
    var screenAppDragStop = function (event, ui) {
        ...
        if (i == -1 || i == data.index) {
            ...
            $el.nextAll().each(function (ix, item) {
                ...
                // ImacScreen.adjustAppPosByIndex(item.getAttribute('data-id'), data.index + ix);
            });
            ...
            if (i < data.index) {
                ...
                $el.nextAll().each(function (ix, item) {
                    ...
                    // ImacScreen.adjustAppPosByIndex(item.getAttribute('data-id'), i + ix + 1);
                });
            } else {
                // 向后移动
                $el.insertAfter($t);
                ...
                $el.prevAll().each(function (ix, item) {
                    ...
                    // ImacScreen.adjustAppPosByIndex(item.getAttribute('data-id'), i - 1 - ix);
                });
            }
...
(function (win, $) {
    ...
    win.TabsNav = {
        ...
        removeAll: function (exceptId) {
            exceptId ?
                this._removeOthers(exceptId) : this._removeAll();
    ...
    $('body').on('click', '.epdialog-hd', function (e) {
        ...
        if (appId) TabsNav.addOrActiveTab(appId);
    });
        ...
        Util.ajax({
            ...
            success: function (data) {
                if (data.remind) {
                    ...
                    $msgNum.text(nNum > 99 ? '99+' : nNum).data('num', nNum).removeClass('hidden');
                } else {
...
(function (win, $) {
    ...
    win.QuickApps = {
        ...
        init: function () {
            ...
            var ids = win.rightUseQuickApps ? Util.localMemory.QuickApps.get() : Util.localMemory.RecentApps.get();
            // renderApps(data);
            if (ids.length) {
        ...
        addApp: function (id, init, isMove) {
            ...
            // 本地缓存中添加 
            win.rightUseQuickApps ? Util.localMemory.QuickApps.add(id) : Util.localMemory.RecentApps.add(id);
            ...
            if (isMove) {
                ...
                var $cloneApp = $fromApp.clone().removeClass('trans').css({
                    'position': 'absolute',
                    'top': pos.top + 50,
                    zIndex: 5000
                }).appendTo('body');
        ...
        removeApp: function (id) {
            ...
            Util.localMemory[win.rightUseQuickApps ? 'QuickApps' : 'RecentApps'].remove(id);
            // 去除draggable功能
            // AppDragMgr.destroy($app);
...
(function (win, $) {
    ...
    // 右侧我的看板
    var $rightBoard = $('#imac-right-bar .right-board-btns'),
        $multiBoardList = $('.multi-board-list', $rightBoard),
     ...
     * 渲染看板
     * 
     * @param {Object} home 门户数据
     * @param {Boolean} isMulti 是否为多门户
     */
    var renderBoard = function (home, isMulti) {
        // 无id时自动生成一个id
        home.id = home.id || +new Date();
        home.url = Util.getRightUrl(home.url);
        var html = Mustache.render(BOARD_BTN_TPL, home);
        $(html).appendTo(isMulti ? $multiBoardList : $rightBoard);
        // 缓存数据
        ImacBoard.data[home.id] = home;
    };
    ...
    var getThemeInfo = function () {
        return Util.ajax({
            ...
            // fullsearch
            var searchForm = document.getElementById('full-search-form');
            if (data.fullSearchUrl) {
                searchForm.action = data.fullSearchUrl;
            } else {
            ...
            // 渲染看板
            if (data.homes) {
                var isMulti = data.homes.length > 1;

                $.each(data.homes, function (i, item) {
                    renderBoard(item, isMulti);
                });
                isMulti && $multiBoardList.prev().removeClass('hidden');
            } else {
                throw new Error('未配置门户');
            }
        });
    };
    ...
    // 我的看板点击
    $rightBoard.on('click', '.right-board-btn', function () {
        var id = this.getAttribute('data-id');
        ImacBoard.show(id);
        $multiBoardList.addClass('hidden');
    }).on('click', '.multi-board-trigger', function () {
        $multiBoardList.toggleClass('hidden');
        ImacBoard.hide();
    });
    $('body').on('click', function (e) {
        if (!$(e.target).closest('.right-board-btns').length) {
            $multiBoardList.addClass('hidden');
        }
    });
    // 点击隐藏
...
(function (win, $) {
    // right
    var $right = $('#imac-right-bar'),
        // 搜索面板
        $searchBox = $('#search-board'),
        // 输入框    
        $searchInput = $('.a-search-input', $searchBox),

        // $closeBtn = $('.close-btn', $searchBox),
        // 所有应用容器
        $allAppsBox = $('#all-apps', $searchBox),
        // 无结果
        $noResult = $('.no-result', $allAppsBox),
        // 搜索关键字
        $searchKey = $('#search-key', $noResult),
        // 第一个tab按钮
        $allAppsTab = $('.tab-item', $searchBox).eq(0),
        // 历史记录容器
...
// 轮训获取应用上的消息数目
(function (win, $) {
    var $container = $('#imac-screens-container');

    /**
     * 发起下一个应用消息数目的请求，索引越界时自动重置
     * 
     * @param {Number} index 下一个应用的索引位置
     * @returns $.ajax 实际下一个的数目请求
     */
    var _getNext = function (index) {
        // 超出则重置
        index = index < AppMgr.msgRemindApps.length ? index : 0;

        // 发起请求 如果已经重置为0 则表示新的一轮开始 需要迟延一轮的时间
        // 否则为下一个 迟延每个的时间
        setTimeout(function () {
            getAppMsgCount(AppMgr.msgRemindApps[index], index);
        }, index === 0 ? (APP_MSG_CONFIG.interval || 60000) : (APP_MSG_CONFIG.space || 50));
    };

    /**
     * getAppMsgCount 获取一个应用上的消息数目
     * 
     * @param {Object} app 要获取消息数目的应用数据，为 AppMgr.msgRemindApps 中存储的某个元素，为了避免每次都查找dom，会在对象中加入对dom的引用
     * @param {Number} index 当前应用在数组中的索引 
     * @returns 
     */
    var getAppMsgCount = function (app, index) {
        return Util.ajax({
            url: app.url,
            data: {
                query: 'getAppMsgCount',
                id: app.id
            }
        }).done(function (data) {
            if (!data) return;

            // var $aimApp = $container.find('[data-id="' + item.id + '"]'),
            //     $count = $aimApp.find('.unread');

            // 避免每次对dom的查找，加入对dom的引用 需要注意 由于存在dom的引用 在卸载应用时清除AppMgr.msgRemindApps中数据时 需要释放对dom的引用
            var $count = app._$count ? app._$count : (app._$count = $container.find('[data-id="' + app.id + '"]').find('.unread')),
                count = data.count;
            ...
            // 避免请求过多时堵塞，在一个完成后并且迟延再发起下一个请求
            _getNext(index + 1);
        });
    };

    var getMsgCount = function (apps) {

        if (!apps.length) return;

        setTimeout(function () {
            getAppMsgCount(apps[0], 0);
        }, APP_MSG_CONFIG.interval);
    };

    // 内部已经实现了一个完成后自动进行下一个，因此不用手动轮训 直接调用即可
    // 不能直接调用  getMsgCount(AppMgr.msgRemindApps); 而应用在所有应用加载完成后进行
    ImacEvent.on('afterAllAppsLoad', function () {
        getMsgCount(AppMgr.msgRemindApps);
    });
...
(function (win, $) {
    // 1、应用仓库打开下时重新加载数据
    // 2、如果存在于本地存储中，将其移除
    // 3、搜索面板中如果存在此dom 需要移除
    // 4、右侧快捷应用或最近使用中存在需要移除dom 右侧是直接初始化的 直接在移除桌面上时同步移除掉了，这里不用处理了
    ImacEvent.on('afterAppUninstall', function (e) {
        // 2、本地数据 搜索记录、右侧快捷或最近使用
        Util.localMemory.SearchHistory.remove(e.data);
        Util.localMemory[win.rightUseQuickApps ? 'QuickApps' : 'RecentApps'].remove(e.data);
        // 3、搜索面板
        if (SearchPanel.isInit) {
            SearchPanel.removeApp(e.data);
        }
        // 1、应用仓库
        var appstore = TabsNav._data['sys-appstore'];
        if (appstore) {
            try {
                appstore.$iframe[0].contentWindow.afterAppUninstall();
            } catch (err) {
                throw err;
            }
        }
