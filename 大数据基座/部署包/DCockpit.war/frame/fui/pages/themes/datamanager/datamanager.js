...
(function (win, $) {
    ...
    _getData_('getPageInfo').done(function (data) {
        ...
        if (data.title) {
            ...
            $("#ei-logoname").html(data.title);
        }
        if (data.logo) {
          $("#ei-logo").attr('src', /^data:/i.test(data.logo) ? data.logo : Util.getRightUrl(data.logo));
        }
        if (data.fullSearchUrl) {
            win._fullSearchUrl_ = data.fullSearchUrl;
        }
...
(function(win,$) {
    ...
    function renderQuickMenu() {
        ...
        quickMenuList.forEach(function(item) {
        	if (menuData.length > 0 && item.items == null) {
                item.items = [];
            }
            var parentCode = $('#all-menu-'+item.code).siblings('.level1').data('id');
            for(var i = 0,len = menuData.length; i < len; i++ ) {
                if(parentCode == menuData[i].code) {
                    var hasmenu = false,
                        activeQuickIndex = 0;
                    for(var j = 0,len1 = quickMenuData.length; j < len1;j++) {
                        if(quickMenuData[j].code == parentCode) {
                            hasmenu = true;
                            activeQuickIndex = j;
                            break;
                        }
                    ...
                    if(hasmenu) {
                        quickMenuData[activeQuickIndex].items.push(JSON.parse(JSON.stringify(item)));
                    } else {
                        quickMenuData.push({
                            code: menuData[i].code,
                            name: menuData[i].name,
                            icon: menuData[i].icon,
                            openType: menuData[i].openType,
    ...
    function activeAllMenu() {
        for(var i = 0; i < quickMenuList.length;i++) {
            $('#all-menu-'+quickMenuList[i].code).addClass('active');
            if(quickMenuList[i].items && quickMenuList[i].items.length) {
                quickMenuList[i].items.forEach(function(item) {
                    $('#all-menu-'+item.code).addClass('active');
                }) 
            }
        }
    ...
    function deleteQuickMenu(id) {
        for (var i = 0; i< quickMenuList.length;i++) {
            if (quickMenuList[i].code == id) {
                ...
                break;
            } else if(quickMenuList[i].items && quickMenuList[i].items.length){
                for(var j = 0,len = quickMenuList[i].items.length;j < len;j++) {
                    if(quickMenuList[i].items[j].code == id) {
                        quickMenuList[i].items.splice(j,1);
                        renderQuickMenu();
                        return;
                    }
                }
    ...
    function saveCommonMenu() {
         ...
         quickMenuList.forEach(function(item) {
        	 ...
        	 if(item.hasOwnProperty("items")){//数组
        		 //遍历
        		 var itemcodes = new Array();
        		 itemcodes = item.items;
        		 for(var i = 0; i < itemcodes.length; i++){
        			 userSelected.push(itemcodes[i].code); 
        		 }
        	 }
        ...
        Util.ajax({
            ...
            data: {
                quickMenuList: JSON.stringify(quickMenuList),
                type: 'user-setting',
    ...
    $allMenuBody.on('click','.allmenu-item-flag',function(e) {
        ...
        if ($menuItem.hasClass('active')) {
            if($menuItem.hasClass('level1')) {
                $menuItem.removeClass('active').siblings().removeClass('active');
            } else {
            ...
            if($menuItem.hasClass('level1')) {
                for(var i = 0,len = menuData.length; i < len; i++ ) {
                    if(id == menuData[i].code) {
                        ...
                        $quickMenuList.append(getTplHtml('quick-menu-tpl',{list: itemData}));
                ...
                parentId = $menuItem.siblings('.allmenu-item').data('id');
                var $activeQuickMenu = $('#quick-menu-'+parentId);
                if($activeQuickMenu.length) {
                    $activeQuickMenu.siblings('.quick-menu-subbox').append(getTplHtml('quick-menu-sub-tpl',{list: itemData}));
                    for(var i = 0,len = quickMenuList.length;i < len;i++) {
                        if(quickMenuList[i].code == parentId) {
                            quickMenuList[i].items.push(itemData);
                    ...
                    for(var i = 0,len = menuData.length; i < len; i++ ) {
                        if(parentId == menuData[i].code) {
                            ...
                            $quickMenuList.append(getTplHtml('quick-menu-tpl',{list: itemDataPar}));
                            quickMenuList.push(itemDataPar);
                            break;
                        }
                    }
                }
                $('#all-menu-'+parentId).addClass('active');
                $menuItem.addClass('active');
            }
        }
        saveCommonMenu();
    ...
    $quickMenuList.on('click','.quick-menu-close',function(e) {
        ...
        var $menuItem = $(this).parent(),
            id = $menuItem.data('id');
            if($menuItem.hasClass('quick-menu-sub')) {
                deleteQuickMenu(id);
                unActiveAllMenu(id);
            } else {
                unActiveAllMenu(id);
                $menuItem.siblings('.quick-menu-subbox').find('.quick-menu-sub').each(function(i,item) {
                    unActiveAllMenu($(item).data('id'));
                })
                deleteQuickMenu(id);
    ...
    .on('click','.quick-menu-sub',function() {
        $allmenuSearchInput.val('');
        searchAllMenu();
