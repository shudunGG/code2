...
(function(win) {
	...
	if (theme == 'grace') {
		...
		skin = 'simple';
	} else if (theme == 'zjgga' || theme == 'leadPage') {
		skin = 'zjgga';
	} else {
		skin = 'default';
	}
	var SrcBoot = {
		isIE8 : !!document.all && document.querySelector
		...
		// 批量输出css|js
		output : function(arr) {
			var i = 0, len = arr.length, id, path, ext;
		...
		getThemeSkins : function(themeName) {
			return this.getThemeConfig(themeName).skins || [];
		},

		_linkTpl : '<link ${id} rel="stylesheet${alternate}" name="${type}" data-name="${name}" ${title} href="${href}">',
		genderLink : function(data) {
			return this._linkTpl.replace(/\${id}/,
					data.id ? 'id="' + data.id + '"' : '').replace(
					/\${alternate}/, data.alternate).replace(/\${title}/,
					data.title).replace(/\${type}/, data.type).replace(
					/\${name}/, data.name).replace(/\${href}/, data.href);
		},
		/**
		 * 输出皮肤资源
		 * 
		...
		outputSkin : function(isTheme, themeName, skins, currSkin) {
			var i = 0, l = skins.length, skin, uiSkinPrefix = 'frame/fui/mutableui/skins/', themeSkinPrefix = 'frame/fui/pages/themes/'
					+ themeName + '/skins/', delayArr = [], isCurr = false, linkStr = '', alternate = '', title = '';
			for (; i < l; i++) {
				skin = skins[i];
				if (skin.disabled) {
					continue;
				...
				linkStr = this.genderLink({
					...
					title : title,
					alternate : alternate,
					href : this.handleResPath(uiSkinPrefix + skin.name
							+ '/skin.css')
				});
				if (isTheme) {
					linkStr += this.genderLink({
						...
						title : title,
						alternate : alternate,
						href : this.handleResPath(themeSkinPrefix + skin.name
								+ '/skin.css')
					});
				}
				// 当前使用的直接输出 其他皮肤迟延加载
				...
				setTimeout(
						function() {
							document.getElementsByTagName('head')[0]
									.insertAdjacentHTML('beforeend', delayArr
											.join(''));
						}, isTheme ? 1000 : 500);
			}
		...
		outputCustomSkin : function(skinPath) {
			var skins = this.getThemeSkins(theme), cskin, delayArr = [], isCurr = false, linkStr = '', i = 0, l = skins.length, alternate = '', title = '';
			...
			for (; i < l; i++) {
				...
				title = isCurr ? '' : 'title="' + (cskin.cname || cskin.name)
						+ '"';
				alternate = isCurr ? '' : ' alternate';
				// linkStr = '<link rel="stylesheet' + (isCurr ? '' : '
				// alternate') + '" name="' + name + '" data-name="' +
				// cskin.name + '" title="' + (cskin.cname || cskin.name) + '"
				// href="' + this.handleResPath(skinPath + cskin.name +
				// '/skin.css') + '"/>';
	...
	function setCookie(name, value, expires, domain) {
		...
		if (expires != null) {
			largeExpDate = new Date(largeExpDate.getTime() + expires * 1000
					* 3600 * 24); // expires天数
		}
		...
		document.cookie = name
				...
				+ (expires == null ? '' : '; expires='
						+ largeExpDate.toGMTString()) + ';path='
				+ (_rootPath ? _rootPath : '/;')
				+ (domain ? '; domain=' + domain : '');
	}

	// 字体大小放大效果实现
	var fontSizeRatio = getCookie('_font_size_ratio_') || 1;
	win.fontSizeRatio = fontSizeRatio;
	var setBaseFontSize = function() {
		document.documentElement.style.fontSize = calcFontSize(
				HtmlBaseFontSize, fontSizeRatio);
		window.addEventListener('DOMContentLoaded', function() {
			document.body.style.fontSize = calcFontSize(BodyBaseFontSize,
					fontSizeRatio);
		});
	};
	...
	if (isThemePage) {
		// 主界面css文件
		arr.push('frame/fui/pages/themes/' + theme + '/' + theme
				+ (SrcBoot.debug ? '' : '.min') + '.css');
	}
	...
	SrcBoot.output(arr);
	// 输出控件皮肤 主界面皮肤
	SrcBoot.outputSkin(isThemePage, theme, SrcBoot.getThemeSkins(theme), skin);
