...
HeaderFilter.prototype = {
    ...
    _createFilterWindow: function (column) {
        var me = this;
        var el = $('<div class="filterwindow mini-popup"><div class="filterwindow-content"></div><div class="filterwindow-footer"><button class="filterwindow-button filter mini-button" noparser>查询</button><button class="filterwindow-button clearfilter mini-button" noparser>清除</button></div></div>').appendTo('body');
    ...
    showFilterWindow: function (column) {
        ...
        var box = mini.getBox(headerCellEl),
            ...
            windowBox = mini.getBox(me.filterWindow[0]);
        if(rightspace < windowBox.width) {
            x = vbox.width - windowBox.width - 16
        }

        me.filterWindow.show();
        mini.setXY(me.filterWindow, x, y);
    },
