...
(function (win, $) {
    // 元件模板
    var WIDGET_TPL = '<li class="widget elem-item" data-id="{{code}}" data-rowspan="{{rowspan}}" data-colspan="{{colspan}}" data-url="{{url}}"><div class="info-box clearfix"><input type="text" class="elem-name" value="{{name}}" readonly="readonly"></div><div class="info-box clearfix"><input type="text" class="elem-url" value="{{url}}" readonly="readonly"></div><div class="elem-btns clearfix"><i class="l elem-remove" title="移除"></i></div></li>';
    ...
    var inputfn = function ($el) {
        $el.find('input')
            ...
            .on('blur', function () {
                ...
                if (!$.trim(value).length) {
                    var that = this;
                    epoint.alert('元件名称不能为空', '系统提醒', function () {
                        that.focus();
                    }, 'warning');
                } else {
                    $this.prop('readonly', 'readonly');
            ...
            mini.confirm('确定移除此元件？', '移除元件', function (action) {
                ...
                Util.ajax({
                    ...
                    if (data.status == 1) {
                        ...
                        epoint.alert(data.description || '移除请求失败！', '', null, 'warning');
                    }
                });
            });
        })
                ...
                Util.ajax({
                    ...
                    if(!data) return;
                     ...
                     if (data.status == 1) {
                        ...
                        epoint.alert(data.description || '禁用请求失败', '', null, 'warning');
                    }
                });
