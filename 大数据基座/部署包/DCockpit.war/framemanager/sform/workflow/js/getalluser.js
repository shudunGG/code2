...
PassStep = $.extend({},
	PassStep, {
		...
		// 当前按钮标识
		$sms_chk: $("#sms_chk", PassStep.$sms),
		$smscontent: $("#smscontent", PassStep.$sms)
	})
...
$(function() {
	...
	// 初始化页面
	epoint.execute('commonoperationhandleaction.getStepInfo', "@none", [PassStep.workitemguid, PassStep.stepguid],
		function(data) {
			render(data);
	...
	var render = function(jsonString) {
		...
		if (!data.smsvisible) {
			...
			PassStep.$sms_chk.prop("checked", data.smschecked);
			PassStep.$smscontent.val(data.smscontext);
		}
		if (data.addhandler) {
			var tabsConfig = [],
				vals = [],
	...
	var bindSelectedEvent = function() {
		$memberlist.on("click", ".removeall",
			...
			}).on('click', '.mini-tabstreeselect-item',
			function() {
...
function getDealData() {
	...
	if ($("#sms").length) {
		data = $.extend(data, {
			'sendsms': PassStep.$sms_chk.prop("checked"),
			'sendsms_jjdd': mini.get("smsjj").getValue(),
			'smscontext': PassStep.$smscontent.val()
		})
	}
	...
	if (PassStep.tabtree) {
		...
		if ($li.length) {
			...
			$li.each(function(i, e) {
				var handlerdata = {};
				handlerdata.handlerguid = $(this).data("handlerguid");
				handlerdata.handlername = $(this).data("handlername");
				handlerdata.ouguid = $(this).data("ouguid");
