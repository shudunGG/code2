...
CKEDITOR.dialog.add( 'cellProperties', function( editor ) {
	...
	var cssContent = window.parent.mini.get("cssContent").getValue();
	var reStr = '\.*[ \s]*\{[^\}]+?\}';
    var re = new RegExp(reStr, "gi");
    var cssAfter = cssContent.match(re);
    var cssItems =[];
 
    for(var i=0; cssAfter != null && cssAfter.length>0 && i<cssAfter.length; i++) {
    	if(cssAfter[i] != null && cssAfter[i].indexOf('.')>-1 && cssAfter[i].indexOf('{')>-1 && cssAfter[i].indexOf('}')>-1){
    		var name = cssAfter[i].substring(cssAfter[i].indexOf('.')+1, cssAfter[i].indexOf('{')-1);
    		var css = [name,name];
    		cssItems.push(css);
    	}  
    }
				...
				children: [ {
					...
					,{
						...
						setup:function( selectedCell ) {
							var a = selectedCell[0].$.getAttribute("class");
							this.setValue(a);
						},
						commit: function( selectedCell ) {
							if ( this.getValue() )
