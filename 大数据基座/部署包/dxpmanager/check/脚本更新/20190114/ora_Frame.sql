...
-- 修改字段长度示例
 begin
  ...
  begin
  select count(1) into isexist from user_tab_columns where table_name = upper('frame_log') and column_name = upper('CONTENT') and data_type='NVARCHAR2' and data_length='2000';
  if (isexist = 0) then
...
-- 修改字段长度示例
 begin
  declare isexist number;
  begin
  select count(1) into isexist from user_tab_columns where table_name = upper('frame_log') and column_name = upper('OPERATECONTENT') and data_type='NVARCHAR2' and data_length='2000';
  if (isexist = 0) then
     execute immediate 'alter table frame_log add newcolumn NVARCHAR2(2000)';
