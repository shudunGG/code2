 ...
 * @Last Modified time: 2019-08-29 14:56:38
 * @Description: '' 
 */
 ...
 */
    ...
    win.getTemplateData = function () {
        Util.ajax({
            ...
            // if (!data || !data.length) {
            //     $tempListContainer.html('<li class="no-result">暂无数据</>');
            //     return;
            // }
...
(function (win, $) {
    ...
    win.gridster = $gridList.gridster({
        ...
        resize: {
            ...
            stop: function (e, ui, $widget) {
                ...
                // var dataInfo = $widget[0].dataset;
                // var _index = getLibIndex(dataInfo.id);
                
                // cacheLibMenu[_index]['other'] = JSON.stringify(dataInfo);

                // $('#lib-item-' + dataInfo.id).attr('data-other', JSON.stringify(dataInfo));
        ...
        draggable: {
            ...
            stop: function (event, ui) {
                // var dataInfo = ui.$player[0].dataset;
                // var _index = getLibIndex(dataInfo.id);
    ...
    var addElementItem = function (data, isInit, noAddLib) {
        ...
        if(data.visible) {
            ...
            //  else {
            //     data.row = Math.ceil(data.height / ROW_H);
            //     // 新增元件自动排布 无位置信息，并直接使用基本尺寸  
            //    gridster.add_widget(html, 10, data.row || MIN_ROWSPAN);
            // }
        }
            ...
            mini.showMessageBox({
                ...
                callback: function (action) {
                    if(action == '确定') {
                        ...
                        // ElementManage.removeElement($li);
                        // var _index = getLibIndex(info.id);
                        // cacheLibMenu.splice(_index, 1);
