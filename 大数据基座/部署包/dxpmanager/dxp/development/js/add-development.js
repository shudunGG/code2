...
var $operationDescription = mini.get("operation-description"), // 操作说明的控件
    ...
    designId = Util.getUrlParams('id'); //Util.getUrlParams('id'); // 如果是修改页面，则url上为已有的ID
...
(function(win, $) {
    var leftNodeData = "";
    console.log(designConfig.leftMenu);
    //渲染左侧数据
    Util.ajax({
...
(function(win, _$) {
    ...
    function init() {
        ...
        if (
            ...
            toNode.data.maxInputLinks &&
            toNodeLinkedList.length >= toNode.data.maxInputLinks
        ) {
            //弹出框
