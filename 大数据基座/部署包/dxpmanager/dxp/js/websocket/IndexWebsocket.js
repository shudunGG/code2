function IndexWebSocket(uid,log,websocketUrl) {
	...
	var handleMsgObj = function(msgObj) {
		if(msgObj.content == 'processEnd'){
			clearInterval(newTimer);
			getLastStepInfo(kettlePort,batchNum,nodeData);
			$funcLog.removeClass('cancel-btn');
			$funcLog.text('运行');
			return;
		}
		log.append(msgObj.content+'</br>');
		log.animate({scrollTop:log.prop("scrollHeight")}, 5);//0.4秒内滚到底部
	};
