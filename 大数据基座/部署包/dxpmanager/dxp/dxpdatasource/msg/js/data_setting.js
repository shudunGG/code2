...
//                $dataChoose.val('').trigger("chosen:updated");
    ...
    function RequestNodeNameData() {
    	$.ajax({
    		url: interfaceNodeName,
    		dataType: "JSON",
    		type: "POST",
    		success: function (data) {
    			data = data.custom;
    			$key2.html(Mustache.render(temp2, data));
    			$key2.chosen({
    				disable_search: true,
    				placeholder_text_single: '请选择所属节点'
    			}).change(function (event, opt) {
    				param.nodename = opt.selected;
...
//                $dataChoose.val('').trigger("chosen:updated");
    ...
    function bindEvent() {
        // 移入移出
        $sqlList.on("mouseenter", ".sql-item>a", function () {
            var that = $(this),
