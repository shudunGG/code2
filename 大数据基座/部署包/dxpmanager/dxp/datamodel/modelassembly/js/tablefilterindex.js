(function(win, $) {
	...
	$filterData.on('click', '.filter-item-add', function() {
		...
		$filterAdd.before(Mustache.render(ITEM_TPL, {
			...
			showFilterRegex : false
		}));
		mini.parse();
		mini.get("pinputds").setData(sourceFieldList);
		mini.get("pinputds1").setData(sourceFieldList);
	...
	win.valueChange = function(e) {
		console.log(e);
		var $parent = $(e.sender.el).parent(), val = e.value, $time = $parent.siblings('.filter-time'), $input = $parent.siblings('.filter-value'), $filterfield = $parent.siblings('.filter-dropdown'), $filterregex = $parent.siblings('.filter-regex');
		...
		if (val == '1') {
			...
			$input.addClass('hidden');
			$filterfield.addClass('hidden');
			$filterregex.removeClass('hidden');
		}
		 else {
			// 其他
			$time.addClass('hidden');
			...
			$filterItem
					.each(function(m, el) {
						var $el = $(el), $filterCategory = mini.byClass('filter-category-select', $el), $filterOperate = mini.byClass('filter-operate-select', $el), $beginTime = mini.byClass(
								...
								'filter-relate-select', $el), $filterField = mini.byClass('filter-dropdown-select', $el),$filterRegex = mini.byClass('filter-regex-select', $el);
						...
						if (mini.get($filterCategory)) {
							var elData = {
								...
								filterRegex : mini.get($filterRegex).getValue() || "",
							}
	...
	win.initHtml = function(data) {
		...
		$(data).each(function(index, ul) {
			...
			$(liList).each(function(m, li) {
				...
				if (li.operateVal == 28) {
					li['showFilterField'] = false;
					li['showFilterRegex'] = true;
					li['hideAll'] = false;
				}
				if (li.operateVal == 1) {
					li['showTime'] = true;
