...
(function($) {
    ...
    var menus = [],
        ...
        arrowElement = $('<span class="menu-item-arrow" />')[0],
        // create main menu div
        $rootDiv = $('<div id="root-menu-div" class="dropdown-menu"></div>'),
        hideTimer,
        defaults = {
            // $.Menu options
            showDelay: 200,
    ...
    $(function() {
        $rootDiv.hover(function() {
            hideTimer && clearTimeout(hideTimer);
        }, function() {
            hideTimer = setTimeout("$.Menu.closeAll()", 100);
        }).appendTo('body');
    });
    ...
    $.extend({
        Menu: function(target, items, options) {
            ...
            this.subMenus = [];
            this.visible = false;
            // this menu has hover or one of its submenus is open
            this.active = false;
    ...
    $.extend($.Menu, {
        checkMouse: function(e) {
            ...
            if (visibleMenus.length && t == visibleMenus[0].target) return;
            ...
            switch (e.keyCode) {
                ...
                case 37:
                    ...
                    if (a && a.parentMenuItem) {
                        ...
                        pmi.$eLI.off('mouseout').off('mouseover');
                        a.hide();
                        pmi.hoverIn(true);

                        // bind again..but delay it
                        setTimeout(function() {
                            pmi.bindHover();
                        });
                ...
                case 39:
                    ...
                    if (a) {
                        ...
                        if (asm && asm.menuItems.length) {
                            ...
                            //select the next menu in the collection
        ...
        prototype: {
            ...
            hide: function() {
                ...
                if (!visibleMenus.length) {
                    $(document).off('mousedown', $.Menu.checkMouse).off('keydown', $.Menu.checkKey);
                }
            ...
            show: function(e) {
                ...
                if (this.menuItems.length) {
                    ...
                    if (ieVersion) {
                        this.$eUL.css('width', ieVersion == 6 ? this.$eDIV.width() - 7 : this.$eUL.width());
                    }
                ...
                if (visibleMenus.length == 0) {
                    $(document).on('mousedown', $.Menu.checkMouse).on('keydown', $.Menu.checkKey);
                }
            ...
            onClick: function(e) {
                if (this.visible) {
                    this.hide();
                    //the class is removed in the hide() method..add it again
                    this.setActive();
                } else {
            ...
            destroy: function() {
                ...
                //unbind events
                if (!this.parentMenuItem) {
                    $(this.target).off('click').off('mouseover').off('mouseout');
                } else {
                    this.$eDIV.off('mouseover').off('mouseout');
                }
    ...
    $.extend($.MenuItem, {
        prototype: {
            ...
            destroy: function() {
                ...
                //unbind events
                this.$eLI.off('mouseover').off('mouseout').off('click');
                //delete submenu
                if (this.subMenu) {
    ...
    var getOneChild = function(elem, name) {
        ...
        for (; n; n = n.nextSibling) {
            if (n.nodeType == 1 && n.nodeName.toUpperCase() == name) return n;
        }
        return null;
    };
