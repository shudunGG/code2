...
(function (doc, Util) {
	 ...
	 * showBtn {HTMLElement} 显示地区选择按钮 or ejs ejs的时候为itemClickNBRightEJS
	...
	function AddrSelect(options) {
		...
		self._hasBtn(options.showBtn, 'showBtn');
	...
	AddrSelect.prototype = {
		...
		_initListeners: function () {
			...
			var showAddrSelect = function () {
				...
				that._showCity(that.provinceIndex, that.cityIndex);
			}

			if (showBtn == 'ejs') {
				window.itemClickNBRightEJS = showAddrSelect;
			} else if (showBtn) {
				that._addEvent(showBtn, '', showAddrSelect);
			}
		 ...
		 * @param {String} key 键
		 */
		_hasBtn: function (dom, key) {
			var self = this;

			if (dom && dom != 'ejs') {
				self[key] = Util.selector(dom);
			} else if (dom == 'ejs') {
				self[key] = 'ejs';
			}
		},
