...
(function(win,$) {
        ...
        sourceList.forEach(function(item) {
            if (item.linkField) {
                ...
                var data = {
                        ...
                        // debugger
                        'data-source': item.field,
                        'data-target': item.linkField,
                        // 'id': 'source-'+ item.field + '_' + 'target-' +
						// item.linkField,
    ...
    function renderLinesColor() {
        ...
        $line.each(function(i,item) {
            var $item = $(item),
            // debugger
            field = $item.attr('data-source'),
            // $source = $('#' + $item.data('source')),
            // $target = $('#' + $item.data('target'));
            $source = get$source($item.data('source')),
            $target = get$target($item.data('target'));
            for(var i = 0; i < sourceList.length;i++) {
    ...
    function renderLinePositon(svgLine) {
        var $svgLine = $(svgLine),
         ...
         // debugger
            $source = get$source($svgLine.data('source')),
            $target = get$target($svgLine.data('target'));
    ...
    function initDeleteBtn() {
        ...
        $deleteBtn.addClass('hidden').data('source','').data('target','');
    }
    // 重置搜索的高亮字段
    ...
    node.on('valuechanged',function(e){
    	...
    	fromtable.setValue("");
    	targetable.setValue("");
    	fromds.setAllowInput(true);
    	targetds.setAllowInput(true);
    	mini.get('viewData').setEnabled(false);
    	epoint.refresh(['fromds','template','targetds','timefield','nodeguid'],function(){
    		epoint.refresh(['fromtable','fromds','targetable','timefield','targetds','nodeguid']);
    	});
    	getTableData();
    	...
    	fromds.setAllowInput(false);
    	targetds.setAllowInput(false);
    	fromtable.setAllowInput(false);
    	targetable.setAllowInput(false);
    	mini.get('viewData').setEnabled(false);
    	epoint.refresh(['fromtable','timefield','fromds','template','targetable','targetds','nodeguid']);
    	getTableData();
    });
    // 选择数据来源
    fromds.on('valuechanged',function(e){
    	...
    	fromtable.setValue("");
    	mini.get('viewData').setEnabled(false);
    	fromtable.setAllowInput(true);
    	epoint.refresh(['fromtable','timefield','template','fromds','targetds','nodeguid']);
    	getTableData();
    }).on('closeclick',function(e){
    	e.sender.setText('');
    	e.sender.setValue('');
    ...
    targetds.on('valuechanged',function(e){
    	...
    	targetable.setValue("");
    	targetable.setAllowInput(true);
    	epoint.refresh(['targetable','template','fromds','targetds','nodeguid']);
    	getTableData();
    }).on('closeclick',function(e){
    	e.sender.setText('');
    	e.sender.setValue('');
    ...
    template.on('valuechanged',function(e){
		...
		mini.get('isTruncate').setVisible(true);
		mini.get('isTruncateFull').setVisible(false);
		mini.get('resetCdcSite').setVisible(false);
    	if(e.value=='003'){
    		// 触发器
    		...
    		mini.get('isTruncate').setVisible(false);
    		Util.form.showField('#model');
			mini.get('transModel').setVisible(false);
			mini.get('timefield').setVisible(false);
			mini.get('isTruncateFull').setVisible(true);
			Util.form.setLabel('#model', '同步规则');
			$('#startTimes').hide();
			mini.get('resetCdcSite').setVisible(false);
    	}else if (e.value == '007') {
			//cdc
			...
			mini.get('isTruncateFull').setVisible(false);
			$('#startTimes').hide();
			Util.form.setLabel('#model', '重置位点');
			mini.get('resetCdcSite').setVisible(true);
		}else{
    ...
    // 同步历史变更
    synHistory.on('valuechanged',function(e){
    	epoint.clear('isTruncate');
    	if(e.value=='Y'){
    ...
    mini.get('isTruncateFull').on('valuechanged',function(e){
    	mini.get('isTruncate').setValue(e.value);
    });
    ...
    scheduler.on('valuechanged',function(e){
    	if(e.value=='1'){
    		...
    		// 定时
    		if(intervalSeconds.getValue()>86400){
        		intervalSeconds.setValue(86400);
    		}
    		if(intervalMinutes.getValue()>1440){
    			intervalMinutes.setValue(1440);
    		}
    		epoint.validate(['intervalSeconds','intervalMinutes']);
    		$('#row1').hide();
    		$('#row2').show();
    		$('#row3').hide();
...
//            field = $row.attr('id').split('-')[1],
    ...
    .on('mouseup',function(e) {
    	// debugger
        if($(e.target).hasClass('checkbox') || $(e.target).hasClass('table-drag') || $(e.target).hasClass('eye-close')) {
            return;
        }
            ...
            for (var i = 0; i < $tableRow.length; i ++) {
                ...
                if (y2 >= top && y2<= (top + 40)) {
                    ...
                    for(var j = 0;j < sourceList.length;j++) {
                        ...
                        if(sourceList[j].linkField == $item.data('field')) {
                            initDragLine();
                            dragFlag = false;
                    ...
                    $dragLine.attr({'data-target':$item.data('field')});
                    break;
                }
            }
            var data = {
...
//                'id': $dragLine.attr('data-source') + '_' + $dragLine.attr('data-target'),
            ...
            linkData($dragLine.attr('data-source'),$dragLine.attr('data-target'))
        } else {
            // debugger
            var $source = get$source($dragLine.attr('data-source'));
            if($source) {
    ...
    .on('click','.delete-btn',function() {
        ...
        var sourceName = $deleteBtn.data('source'),
            targetName = $deleteBtn.data('target'),
            $source = get$source(sourceName),
            $target = get$target(targetName);
        // $('#' + lineId).remove();
        $('.line').each(function(i,item) {
            var $item = $(item);
            if($item.attr('data-source') == sourceName && $item.attr('data-target') == targetName) {
                $item.remove();
            }
        })
    ...
    .on('click','.link-row',function() {
        ...
        for (var i = 0; i < length; i++) {
        	//debugger
            var sourceName = $tableLeftRow.eq(i).data('field'),
                targetName = $tableRightRow.eq(i).data('field');
