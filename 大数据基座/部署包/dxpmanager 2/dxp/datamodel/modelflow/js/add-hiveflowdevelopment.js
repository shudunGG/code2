/**
 * ! 数据开发 date:2019-10-14 author: huangweiping;
 */
...
var $operationDescription = mini.get("operation-description"), // 操作说明的控件
    ...
    designId = Util.getUrlParams('flowGuid'); // Util.getUrlParams('id'); //
												// 如果是修改页面，则url上为已有的ID
...
(function(win, $) {
    ...
    //20210220修改展开收起
    $('body').on('click', '.type-title', function () {
        var $this = $(this);
        $this.toggleClass('active');
        $this.next().toggleClass('hidden');
        $('.bdl').getNiceScroll().resize();
    });
    ...
    $searchBtn.on("click", function() {
        ...
        if (searchData) {
            ...
            $.each(leftNodeData.navList, function(i, item) {
                $.each(item.typetitle, function(i, newitem) {
                    $.each(newitem.list, function(i, newitem1) {
                        if (newitem1.name.indexOf(searchData) >= 0||newitem1.introduce.indexOf(searchData) >= 0) {
                            var newNodeData = mini.clone(newitem1);
...
(function(win, $) {
    ...
    $("body")
        ...
        .on("click", ".cxm", function(event) {
            ...
            var $this = $(this),
                type = $this.data("type");
            if (type == "main") {
            	designConfig.onAssemblySqlPreviewClick(newlinkData);
            } else if (type == "previewdata") {
            	console.log(newlinkData);
            	designConfig.onAssemblyDataPreviewClick(newlinkData);
            }
    ...
    $designHeader
        ...
        .on("click", "#func-quickformat", function() {
        	if (
                    designConfig.onQuickformat &&
                    typeof designConfig.onQuickformat == "function"
                ) {
        		designConfig.onQuickformat();
        	}
        })
        .on("click", "#func-sql", function() {
        	if (
        			designConfig.onSqlPreviewClick &&
        			typeof designConfig.onSqlPreviewClick == "function"
        	) {
        		designConfig.onSqlPreviewClick();
        	}
        })
...
(function(win, _$) {
    ...
    function init() {
        var dragged = null; // A reference to the element currently being
							// dragged

        // highlight stationary nodes during an external drag-and-drop into a
		// Diagram
        ...
        div.addEventListener(
            ...
            function(event) {
                ...
                if (this === myDiagram.div) {
                    ...
                    // if the target is not the canvas, we may have trouble, so
					// just quit:
                    if (!(can instanceof HTMLCanvasElement)) return;
                    ...
                    var itemData = {
                        ...
                        maxInputLinks: dragged.dataset.maxinputlinks,
                        tablename:dragged.dataset.tablename,
                        tableid:dragged.dataset.tableid,
                        partiontype:dragged.dataset.partiontype,
                        loc: loc,
                        grouptype: dragged.dataset.grouptype,
                        modeltype: dragged.dataset.modeltype,
                        isneedpartion: dragged.dataset.isneedpartion
                    };
        ...
        myDiagram.nodeTemplate = $(
            ...
            $(
                ...
                {
                    name: "text",
                    alignment: new go.Spot(0, 1, 30, 25),
                    editable: false,
                    isMultiline: true,
                    width: 110,
            ...
            $(
            		...
            		{
            			name: "text",
            			alignment: new go.Spot(0, 0,30, -10),
            			editable: false,
            			isMultiline: true,
            			width: 110,
            			// margin: new go.Margin(55, 0, 5, 0),
            ...
            ),
            {
                click: function(e, node) {
                    ...
                    if (
                            designConfig.onSelect &&
                            typeof designConfig.onSelect == "function"
                        ) {
                            designConfig.onSelect(node.data);
                        }
                },
                ...
                if (selectedObj.data.linksQueue) {
                    ...
                    for (var i = 0, l = linkArray.length; i < l; i++) {
                        ...
                        if(toNode==null){
                        	...
                        	var  _toLinkQueue = toNode.data.linksQueue || "[]",
                    ...
                    if(toNode.data.type!="com.epoint.dxp.development.model.flow.steps.TableOutputStep"){
                    epoint.execute("getCountByKey", '',[toNode.data.key], function(msg) {
                    	if(msg!=''){
                    		msg=msg+"条";
                    	}
                        myDiagram.model.setDataProperty(
