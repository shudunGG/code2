...
(function (win, $) {
    ...
    function reLayoutApp() {
        var newColNum = (($(win).height() - OCCUPIED_H - APP_INIT_TOP) / (APP_HEIGHT + APP_GAP)) >> 0;
        // 高度变化未导致展示数目变化时无需处理
    ...
    win.AppMgr = {
        ...
        moveToScreen: function (id, page) {
            // 目标屏幕最后一个应用
            var $prevApp = ImacScreen.getScreen(page)
                .find('.app-item')
                .last(),
        ...
        installApp: function (id, callback) {
            // 当前屏幕最后一个应用
            var $prevApp = ImacScreen.getScreen(activePage)
                .find('.app-item')
                .last(),
                prevAppId = $prevApp.length ? $prevApp.data('id') : null;
...
(function (win, $) {
    ...
    function initEvent(defaultHome) {
        ...
        // 门户面板高度处理
        var timer,
            $portalPanel = $portal.find('.portal');
        $(win)
            .on('resize', function () {
                clearTimeout(timer);
                timer = setTimeout(function () {
