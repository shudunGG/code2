(function(win, $) {
	...
	$.extend(Flow.prototype, {
		...
		_active: function() {
			...
			var $sibling = $flow.removeClass('default done disabled error')
				.addClass('active')
				.siblings('.active')
				.removeClass('active');
	...
	var initWizard = function(data) {
		$wizardBar.addClass('top-wizard2')
		// 缓存流程数据
		flowData = data;
	...
	$wizardNav.on('click', '.wizard-step', function(event) {
		...
		if($el.hasClass('save')) {
			callback && $.proxy(callback, this, 'save', curFlow)();
			return;
		}
		
		if($el.hasClass('submit')) {
			callback && $.proxy(callback, this, 'submit', curFlow)();
			return;
		}
		
		if($el.hasClass('know')) {
			callback && $.proxy(callback, this, 'know', curFlow)();
			return;
		}
