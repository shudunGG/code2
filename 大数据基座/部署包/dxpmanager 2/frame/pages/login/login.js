			...
			.extend(
					TabView.prototype,
					{
						_initView : function() {
							...
							$widgetHd = $widget.find('> [data-role="head"]'), $widgetBd = $widget
									.find('> [data-role="body"]'),

							$tabs = $widgetHd.find('[data-role="tab"]'), $tabCons = $widgetBd
						...
						_initEvent : function() {
							var c = this.cfg, triggerEvent = c.triggerEvent,

							$widgetHd = this.$widgetHd, self = this;

							// 用于mouseover触发时的延时
							var overTimer = 0;

							if (triggerEvent == 'click') {
								$widgetHd.on('click', '[data-role="tab"]',
										function(event) {
											event.preventDefault();

											$.proxy(self._activeTab, self,
													$(this))();
										});
								...
								$widgetHd.on(
										...
										function() {
											...
											overTimer = setTimeout($.proxy(
													self._activeTab, self,
													$(this)), 500);
						...
						_activeTab : function($tab) {
							var c = this.cfg, activeCls = c.activeCls;
						...
						activeTabByIndex : function(index) {
							...
							$activeTab = null, targetId = '';

							// 若index合法
							if (index >= 0 && index < $tabs.length) {
								$activeTab = $tabs.removeClass(activeCls).eq(
										index).addClass(activeCls);
						...
						_activeTabContent : function(targetId) {
							...
							$tabCons.addClass('hidden').filter(
									'[data-id="' + targetId + '"]')
									.removeClass('hidden');
						}
					});
	...
	if (initUrl) {
		Util.ajax({
			...
			success : function(data) {
				if (data) {
					var title = data.title, logoImg = data.logoImg;
