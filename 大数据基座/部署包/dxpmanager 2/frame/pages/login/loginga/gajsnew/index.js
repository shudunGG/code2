...
var initClickEvent = function() {
    ...
    win.encryUserMsg = function() {
        ...
        if ('0' == loginType) {
            ...
            if(!isCheck){
				epoint.showTips('请阅读并勾选《新点数据建模平台隐私保密声明》', {
                    state: 'warning'
                });
				return false;
			}
        ...
        epoint.execute('loginaction.login', '', [userName, passWord,
            loginType, rememberMe, verifycode
        ], checkMultipleLogin);
    ...
    win.secondLogin = function(result) {
        if (result && result != 'success' && result != 'close') {
            ...
            epoint.execute('loginaction.userLogin', '', [loginType, msgs[1],
                    msgs[2], epoint.encode(userName), epoint.encode(passWord), rememberMe, verifycode
                ],
                userLoginCallBack);
...
var $verityCode = $('#verifycode-box'),
    $usbVerityCode = $('#usbverifycode-box'),
    $login = $('#brainLogin'),
    $cube = $('#cubeTransition'),
    $tabview = $('#tabview');
...
$(function() {
                ...
                success: function(data) {
                    if (data) {
                        var title = data.title,
                            ...
                            showDriveDownload = data.showDriveDownload === undefined ? true :
                            data.showDriveDownload,
                            showMobileOADownload = data.showMobileOADownload === undefined ? true :
    ...
    var cubeTransition = function() {
        ...
        function show() {
            ...
            // 以下为0524新增
            $('.login-title').eq(current - 1).hide();
            $('.login-title').eq(next - 1).show();
            // 以上为0524新增
        }
        ...
        if (gather.type == 'IE') {
            ...
            $('#cubeTransition').on(
                ...
                function() {
                    params.type = 1;
                    setCookie();
                    $('.login-way').eq(1).removeClass('hidden').siblings()
                        .addClass('hidden');
                }).on(
                'click',
                '#brain',
                function() {
                    params.type = 0;
                    setCookie();
                    $('.login-way').eq(0).removeClass('hidden').siblings()
                        .addClass('hidden');
                });

        } else {
            $('.login-way').removeClass('hidden');
            cubeTransition();
        }
    };
    ...
    $accountLogin
        .validate({
            errorPlacement: function(error, element) {
                var $infoError = $('.infoError', $accountLogin),
                    id = element[0].id,
                    text = error[0].innerText;
    ...
    $phoneLogin
        .validate({
            errorPlacement: function(error, element) {
                var $infoError = $('.infoError', $phoneLogin),
                    id = element[0].id,
                    text = error[0].innerText;
