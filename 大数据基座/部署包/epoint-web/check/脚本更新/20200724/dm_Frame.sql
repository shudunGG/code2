...
-- 修改table_basicinfo表mergeformids字段类型改为longtext
 ...
 begin
  ...
  begin
  select count(1) into isexist from user_tab_columns where table_name = upper('table_basicinfo') and column_name = upper('mergeformids') and data_type='CLOB' ;
  if (isexist = 0) then
...
/* GO */
 ...
 begin
  declare isexist number;
  begin
  select count(1) into isexist from user_tab_columns where table_name = upper('epointsform_tablerelation') and column_name = upper('businesstableid') and data_type='CLOB' ;
  if (isexist = 0) then
