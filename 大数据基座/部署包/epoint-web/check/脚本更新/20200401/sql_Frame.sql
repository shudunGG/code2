...
select @check= name from dbo.sysobjects where id = object_id(N'[dbo].[CreatePVI]') and OBJECTPROPERTY(id, N'IsProcedure') = 1
IF @check is not  null
DROP PROCEDURE [dbo].[CreatePVI]
GO
...
SELECT
	@T_ProcessVersionGuid=ProcessVersionGuid,
	@T_ProcessVersionName=ProcessVersionName 
...
/******新增时间=当前时间 ******/
SET @T_AddDate = GETDATE();

/******材料id ******/
SET @T_MaterialInstanceGuid = newid();

/******开始活动实例 ******/
SET @T_StartActivityInstanceGuid = newid();

/******开始活动Guid ******/
SELECT
	@T_StartActivityGuid=ActivityGuid,
	@T_StartActivityName=ActivityName 
...
/******第一步流程活动实例Guid ******/
SET @T_FirstActivityInstanceGuid = newid();

/******第一步流程活动Guid,活动名称， ******/
SELECT
	@T_TransitionGuid=TransitionGuid,
	@T_FirstActivityGuid=ToActivityGuid,
	@T_WorkItemModeName=WorkItemModeName 
...
/******第一步流程活动名称 ******/
SELECT
	@T_FirstActivityName=ActivityName,
	@T_HandleURL=HandleURL,
	@T_Priority=Priority 
