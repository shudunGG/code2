...
-- api_info添加gkyUrl字段 --【陈端一】
 begin
  declare isexist number;
  begin
  select count(1) into isexist from user_tab_columns where table_name = upper('api_info') and column_name = upper('gkyUrl');
  if (isexist = 0) then
    execute immediate 'alter table api_info add gkyUrl nvarchar2(10)';
  end if;
  end;
end;
/* GO */
