...
/*
 * global METRO_SETTINGS: true, EpDialog, _MetroEvent_, MetroDataActionUrl,
 * AppMgr
 */
...
(function (win, $) {
    ...
    win.AppMgr = {
        ...
        // 安装应用
        installApp: function (data, callback) {
        ...
        moveToScreen: function (id, page) {
            ...
            if (idx == -1) {
                ...
                // onlyOkTip.setMessage('很抱歉，第' + (page + 1) +
				// '分屏没有足够的空间，您可以选择其他分屏或者调整应用布局后再移动。')
                // .setType('warn')
                // .show(true);
            } else {
                data.index = idx;
...
(function (win, $) {
    ...
    function doSearch(type, kw) {
        ...
        // } catch (error) {}
        win.open(Util.getRightUrl(win._fullSearchUrl_ + '?wd=' + win.encodeURI(kw) + '&type=' + type));
    }
