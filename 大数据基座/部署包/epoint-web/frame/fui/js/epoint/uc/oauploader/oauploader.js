...
mini.extend(mini.OAUploader, mini.UserControl, {
    ...
    init: function () {
        ...
        url = Util.adjustRequestContextCmd(url);
        ...
        var handelUploadSuccess = function(e) {
            ...
            if (fileGuid) {
                ...
                $fileItem.find('.mini-uploader-progressbar').hide();
            }
        };

        // 服务器端成功接收文件后，把服务端返回的fileId设置到对应的dom上。并设置下载url
        this._mWebUploader.on('uploadAccept', handelUploadSuccess);
        // 分片的实际上传成功事件
        this._mWebUploader.on('finishedmd5file', handelUploadSuccess);
    ...
    setEnabled: function (value) {
        mini.OAUploader.superclass.setEnabled.call(this, value);
        this._mWebUploader && this._mWebUploader.setEnabled(value);
    },
