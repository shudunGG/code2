...
epctrl.WeekCalendar = function (cfg) {
    ...
    this.setCss('frame/fui/js/widgets/epcontrols/weekcalendar/weekcalendar.css');

    // 当前无moment时 自动引入moment
    if (!window.moment || typeof window.moment != 'function') {
        this.setJs('frame/fui/js/libs/moment.min.js');
    }
...
epctrl.extend('WeekCalendar', 'Control', {
    ...
    // 一周的天数
    // _WEEKDAYS: 7, // 新增隐藏周末，此处需要计算得出

    // 一周分钟数
    _WEEKMINUTES: 7 * 24 * 60, // 新增隐藏周末，此处需要计算得出，保留原值以兼容未更新的情况
    // 一周秒数
    _WEEKSECONDS: 7 * 24 * 3600, // 新增隐藏周末，此处需要计算得出，保留原值以兼容未更新的情况
    // 一天的分钟数秒数
    _DAYMINUTES: 24 * 60,
    _DAYSCONDS: 24 * 3600,
    ...
    _defaultCfg: {
        ...
        // 隐藏周末 隐藏后周末不在展示，要求数据中也不能含有包含周末的数据
        hideWeekends: false,
    ...
    _initCfg: function () {
        ...
        if (!this.cfg.el || !jQuery(this.cfg.el).length) {
        ...
        // 隐藏周末
        this._setHideWeekends(this.cfg.hideWeekends);
    },

    _setHideWeekends: function (v) {
        v = v === true ? true : false;
        if (v === this.hideWeekends) {
            return false;
        }
        this.hideWeekends = this.cfg.hideWeekends = v;
        if (this.hideWeekends) {
            // 隐藏周末时，只支持从周一开始
            this.dayStartFromSunday = this.cfg.dayStartFromSunday = false;
        }
        this._calculateForWeek();
    },
    // 新增隐藏周末，时间长度需要进行计算
    _calculateForWeek: function () {
        this._WEEKDAYS = this.hideWeekends ? 5 : 7;
        // 一周分钟数
        this._WEEKMINUTES = 60 * 24 * this._WEEKDAYS;
        // 一周秒数
        this._WEEKSECONDS = 3600 * 24 * this._WEEKDAYS;
    },

    setHideWeekends: function (v) {
        this._setHideWeekends(v);
        this._renderGrid();
        this._renderChanged();
    },
    getHideWeekends: function () {
    ...
    _rednerContent: function () {
        ...
        while (i < this._categoryData.length) {
            ...
            style = this._gridEl.children[i].style.cssText;
            style && row.setAttribute('style', style);
    ...
    _renderChanged: function () {
        this.$el[this.hideWeekends ? 'addClass' : 'removeClass']('no-weekends');
        // 清空内容
        this._clearContent();
        // 初始日期范围
        // 渲染标题和日期
    ...
    _renderCalendarTitle: function () {
        var start_text = this._startDate.format('YYYY年MM月DD日'),
            end_text =
            this._endDate.format(this._startDate.isSame(this._endDate, 'month') ? 'DD日' : 'MM月DD日');
    ...
    _initEvent: function () {
        ...
        jQuery(this.el)
            ...
            .on('mousedown.weekcalendar', '.ep-weekcalendar-content-row', function (e) {
                ...
                columnWidth = jQuery(me._gridEl).width() / me._WEEKDAYS;
        ...
        jQuery('body')
            ...
            .on('mouseup.weekcalendar', function (e) {
                ...
                columnEnd = columnEnd > me._WEEKDAYS - 1 ? me._WEEKDAYS - 1 : columnEnd;
    ...
    _getNumByUnits: function (dateStr) {
        ...
        var times = (temp[1] || '').split(':'),
            days = (function (startDate) {
                ...
                // debugger
                console && console.error && console.error('计算天数时出现异常！【' + date + '】');
                return i;
            }(this._startDate)),
    ...
    addWidget: function (data) {
        var rowIndex = this._categoryReocrds[data.categoryId];
        var row = this._contentEl.childNodes[rowIndex];
    ...
    // 设置行高度
    setRowHeight: function (height, rowIndex) {
        var $cateItems = $(this._categoryListEl).children(),
            $contentRows = $(this._contentEl).children(),
            $gridRows = $(this._gridEl).children();

        if (rowIndex != undefined) {
            $cateItems.eq(rowIndex).css('height', height);
            $contentRows.eq(rowIndex).css('height', height);
            $gridRows.eq(rowIndex).css('height', height);
        } else {
            $cateItems.css('height', height);
            $contentRows.css('height', height);
            $gridRows.css('height', height);
        }
    },
