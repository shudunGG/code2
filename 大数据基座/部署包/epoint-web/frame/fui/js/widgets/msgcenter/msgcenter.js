 ...
 * @Last Modified time: 2020-10-10 13:44:57
 * @Description: '消息中心' 
 */
(function (win, $) {
    var defaultConfig = {
    ...
    var MSGBOX_TPL = '<div id="mc-wrapper"><div class="mc-header" id="mc-header">{{title}}' +
        ...
        '<input type="text" class="msg-search-input l" autocomplete="off" id="msg-search-input" placeholder="请输入标题或发送人" />' +
        ...
        TYPE_TPL = '<ul>{{#msgTypes}}<li data-id="{{id}}" class="msg-tab {{#show}}active{{/show}}"><i class="msg-tab-icon {{icon}}" style="background-color: {{bg}}"></i><span class="msg-tab-text" title="{{name}}">{{name}}</span> <i id="msg-num-{{id}}" class="msg-num-tip hidden"></i></li>{{/msgTypes}}</ul>',

        MSGITEM_TPL = '{{#list}}{{#isTime}}<li class="msg-time-item"><span class="msg-time-tip">{{time}}</span></li>{{/isTime}}{{^isTime}}<li id="msg-item-id-{{id}}" class="msg-item {{#readed}}msg-readed{{/readed}} clearfix"><div class="msg-check-btn msg-item-check" {{#emailId}}data-emailid="{{emailId}}"{{/emailId}} data-id="{{id}}"></div><div class="msg-item-container"><div class="msg-item-top clearfix"><h3 class="l msg-item-type"><i class="msg-tab-icon {{icon}}" style="background-color: {{bg}}"></i><span class="msg-type-name">{{typeName}}</span></h3><span class="r">{{showTime}}</span></div><div data-opentype="{{openType}}" data-id="{{id}}" {{#emailId}}data-emailid="{{emailId}}"{{/emailId}} data-name="{{title}}"  data-url="{{url}}" data-typeid="{{typeId}}" data-readed={{readed}} class="msg-item-title">{{{content}}}</div><div class="msg-item-more"><div class="msg-item-link text-ellipsis" data-opentype="{{openType}}" data-id="{{id}}" {{#emailId}}data-emailid="{{emailId}}"{{/emailId}} data-name="{{title}}"  data-url="{{url}}" data-typeid="{{typeId}}" class="msg-item-link">{{{title}}} ></div>{{#showIgnore}}<div class="msg-ignore r" title="设为已读" data-id="{{id}}" {{#emailId}}data-emailid="{{emailId}}"{{/emailId}}></div>{{/showIgnore}}</div></div></li>{{/isTime}}{{/list}}';
    ...
    $.extend(MsgCenter.prototype, {
        ...
        _initDomObj: function () {
            ...
            this.$editContainer = $('#msgopera-edit-container'); // 编辑内容
        },
        ...
        _initEvent: function () {
            ...
            this.$msgContent.on('click', '.msg-edit-btn', function () {
                ...
                that.$editContainer[$('.msg-item-check.active').length ? 'addClass' : 'removeClass']('active');
        ...
        _msgListRequest: function (opt, callback) {
            ...
            that.msglistAjax = Util.ajax({
                ...
                if (res && res.list.length) {
                    ...
                    that.$msgLists.removeClass("list-loading").addClass('msg-no');
                }
            });
        ...
        _adjustMsgList: function (list) {
            ...
            $(list).each(function (index, item) {
                if (item.createTime && !item.isTime && !item.isTop) {
        ...
        _renaderMsgList: function (list, isChangeType) {
            ...
            if (isChangeType) {
                that.$msgListsUl.html(Util.clearHtml(M.render(MSGITEM_TPL, {
                    list: list
                })));
            } else {
                that.$msgListsUl.append(Util.clearHtml(M.render(MSGITEM_TPL, {
                    list: list
                })));
            }
