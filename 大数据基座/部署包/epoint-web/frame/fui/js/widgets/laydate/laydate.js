...
;!function(){
  ...
  var isLayui = window.layui && layui.define, ready = {
    ...
    ,link: function(href, fn, cssname){
      // 为了控件可以换肤，css不能懒加载插入到head的最后
      // 已将css放到了miniui控件的css文件中，这里只需直接执行回调
  ...
  */
  ...
  Class.prototype.calendar = function(value){
    ...
    lay.each(tds, function(index, item){
      ...
      typeof options['drawdate'] === 'function' && options['drawdate'].apply(options, [param, item]);
    });  
  ...
  Class.prototype.list = function(type, index){
    ...
    if(type === 'year'){ //年列表
      ...
      if(options.range){
        ...
        //   ,seconds: 0
        // };
        // modify by liub @ 2021/02/01
        // 修改默认 endTime 为 23:59:59
        if(startEnd === 'endTime') {
          that[startEnd] = {
            hours: 23
            ,minutes: 59
  ...
  Class.prototype.choose = function(td){
    ...
    if(options.range){
      ...
      // add @2020/11/9 解决先选择开始日期的时、分、秒大于结束日期的时、分、秒，再选择开始日期和结束日期为同一天，能选出开始时间大于结束时间的问题
      //同步按钮可点状态
      that.setBtnStatus();
  ...
  Class.prototype.events = function(){
    ...
    ,showEvent = function(elem, bind){
      elem.on(options.trigger, function(){
        ...
        that.config.readOnly || !that.config.enabled || that.render();
      });
    };
