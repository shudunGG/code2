...
function Protect(){
    TANGER_OCX_OBJ.ActiveDocument.protect(0,0,'');
}
...
function getActiveDocument(){
	return TANGER_OCX_OBJ.ActiveDocument;
}
...
function EnterRevisionMode() {
    TANGER_OCX_OBJ.EnterRevisionMode();
}


//编辑文档
function OpenEditOffice(url, newofficetype) {
    if(!TANGER_OCX_OBJ) TANGER_OCX_OBJ = document.all("TANGER_OCX"); //;初始化控件对象
...
//保存office文档
function saveFileToUrl(savrUrl,fileName,guid,saveType,exportFileCallBack) {
    var result, filedot;
    if(!saveType){
    	saveType = '';
    }
    if (IsFileOpened) {
		...
		"DocContent", //文件域的id，类似<input type=file id=upLoadFile 中的id
		"para="+guid+"&fileName="+encodeURI(fileName)+"&filedot="+filedot+"&savetype="+saveType+"&fileType=" + fileType,          //与控件一起提交的参数,savetype参数为要保存的文件格式office,html,pdf。filetype参数保存文件类型
    ...
    if (IsFileOpened) {
        if (TANGER_OCX_OBJ.doctype == 1 || TANGER_OCX_OBJ.doctype == 2) {
            try {
				...
				0, //left 左边距
				0, //top 上边距 根据Relative的设定选择不同参照对象
	...
	function initCustomMenus1() {
	    ...
	    for (var menuPos = 0; menuPos < 1; menuPos++) {
	        ...
	        for (var submenuPos = 0; submenuPos < 1; submenuPos++) {
	            try { myobj.AddCustomMenuItem2(menuPos, submenuPos, -1, true, "打印设置", false); } catch (err) { };
	            //增加子菜单项目
