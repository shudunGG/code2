(function (win, $) {
    var TABS_ITEM_TPL = '<li class="tabsnav-tabs-item{{^closeIcon}} fixed{{/closeIcon}}" id="tab-{{id}}" data-id="tab-{{uid}}" data-target="tab-content-{{id}}" {{#autoWidth}}style="width:auto;"{{/autoWidth}}><span class="tabsnav-tabs-name" title="{{name}}" {{#autoWidth}}style="width:auto;"{{/autoWidth}}>{{name}}</span>{{#refresh}}<i class="tabsnav-tabs-refresh"></i>{{/refresh}}{{#closeIcon}}<i class="tabsnav-tabs-close"></i>{{/closeIcon}}</li>',
        ...
        CONTEXT_MENU_TPL = '<div class="tabsnav-contextmenu hidden" id="{{id}}"><ul>{{#items}}{{#text}}<li class="tabsnav-contextmenu-item" role="{{role}}" title="{{text}}">{{text}}</li>{{/text}}{{^text}}<li class="tabsnav-contextmenu-item sep"></li>{{/text}}{{/items}}</ul></div>',
    ...
    var defaultConfig = {
        ...
        tabWidth: 112 // 默认一个tab的宽度, 配置为0标识自适应
    };
    ...
    $.extend(EpTabsNav.prototype, {
        _init: function () {
            ...
            this.contextMenu = new ContextMenu({
                onHide: function () {
                    self._hidePageCover('contextMenu');
                },
                items: [
                    {
                        text: '关闭',
        ...
        _bindEvent: function () {
            ...
            this.$tabsWrapper
                .on('click', '.tabsnav-tabs-item', function (e) {
                    // 点击激活
                    var id = this.id;
                    self.activeTab(id);
                })
                ...
                .on('click', '.tabsnav-tabs-refresh', function (e) {
                    // 刷新
                    // 如果阻止事件冒泡会导致tab在未激活状态下刷新，从而导致刷新后页面布局错乱
                    // e.stopPropagation();
                ...
                .on('contextmenu', '.tabsnav-tabs-item', function (event) {
                    ...
                    self._showPageCover('contextMenu');
        ...
        addTab: function (cfg, withoutActive) {
            ...
            if (!this.tabsCache[tabId]) {
                // 处理url，补全路径
                if (!withoutActive) {
                    data.url = Util.addHtmlToUrl(Util.getRightUrl(data.url));
                } else {
                    data.realUrl = Util.addHtmlToUrl(Util.getRightUrl(data.url));
                    data.url = 'about:blank';
                }

                // tab模板要渲染到两个地方 id不能相同 uid用以标识其二者相同的id
                data.uid = data.id;
                data.autoWidth = this.cfg.tabWidth === 0;
        ...
        activeTab: function (id) {
            ...
            if ($tab.length) {
                ...
                $tabCon.removeClass('hidden').siblings('.tab-content').addClass('hidden');
        ...
        adjustTabPosOnActive: function (id) {
            ...
            var self = this,
                ...
                tab_w = this.cfg.tabWidth === 0 ? $li.outerWidth(true) : this._getTabWidth(),
                scroll_w = Math.ceil(Math.abs(parseFloat(this.$variableTabsList.css('margin-left')))),
                marginLeft = scroll_w,
                // prev_w = tab_w * $prev_lis.length,
                prev_w = this._calcTabWidth($prev_lis),
                max_w = this._max_w,
                tabs_w = this._tabs_w;
            ...
            if (tabs_w <= max_w) {
                // 可视区域大于tabs宽度，不需滚动
                marginLeft = 0;
            } else if (prev_w <= scroll_w) {
                // 激活tab在左侧隐藏部分，则需将激活tab移到可视区域有最左侧
                marginLeft = prev_w;
            } else if (prev_w >= max_w + scroll_w || prev_w + tab_w > max_w + scroll_w) {
                // 激活tab在右侧隐藏部分，则需将激活tab移到可视区域最右侧
                marginLeft = prev_w + tab_w - max_w;
            } else if (tabs_w - scroll_w < max_w) {
                // 存在滚动并且tabs的最右侧在可视区域最右侧的左边，则需要将tabs的最右侧移到可视区域的最右侧
        ...
        _getTabWidth: function () {
            if (!this._tab_w) {
                this._tab_w = this.$fixedTabsList.children().eq(0).outerWidth(true) || this.cfg.tabWidth || 112;
            }

            return this._tab_w;
        },

        _calcTabWidth: function ($list) {
            if (this.cfg.tabWidth !== 0) {
                this._calcTabWidth = calcFixedSize;
                return calcFixedSize.call(this, $list);
            }
            this._calcTabWidth = calcAutoSize;
            return calcFixedSize.call(this, $list);

            // 指定宽度则 unit * num
            function calcFixedSize($list) {
                if (!$list) {
                    $list = this.$variableTabsList.children();
                }
                return this._getTabWidth() * $list.length + 1;
            }

            // 自适应则逐个计算
            function calcAutoSize($list) {
                if (!$list) {
                    $list = this.$variableTabsList.children();
                }
                return (
                    $.map($list, function (tabEl) {
                        return $(tabEl).outerWidth(true);
                    }).reduce(function (p, c) {
                        return p + c;
                    }, 0) + 10
                );
        ...
        _calcSize: function () {
            ...
            if (this.cfg.needScrollBtn) {
                ...
                this._scroll_w_once = this.cfg.tabWidth === 0 ? 112 * this.cfg.smoothItems : this._tab_w * this.cfg.smoothItems;
            }
        ...
        _showQuickNavBox: function () {
            ...
            if (this.cfg.position == 'bottom') {
                attr = {
                    bottom: $tabContainer.outerHeight(true) + 1
                };
                this.$quickNavBox.css('top', 'auto');
            } else {
                attr = {
                    top: $tabContainer.outerHeight(true) + 1
                };
        ...
        _adjustScrollBtnVisible: function () {
            ...
            if (max_w > tabs_w) {
                ...
                ml = Math.ceil(Math.abs(parseFloat(this.$variableTabsList.css('margin-left'))));
                max_ml = tabs_w - max_w;
        ...
        _pageCoverTarget: '',
        _showPageCover: function (target) {
            this._pageCoverTarget = target;
            this._getPageCover().removeClass('hidden');
        },
        // 由于存在两处控制pagecover显隐的地方，需要用个标识区分下
        _hidePageCover: function (target) {
            if (this._pageCoverTarget == target) {
    ...
    ContextMenu.prototype = {
        ...
        show: function (pos) {
            this.$widget
                ...
                .removeClass('hidden');
            this.cfg.onShow && this.cfg.onShow();
