...
(function (win, $) {
    ...
    CommonDto.prototype = {
                        ...
                        if (value !== undefined && control.type != 'treegrid') {
                            ...
                            if (control.type == 'webeditor') {
                                ...
                                if (Util.getFrameSysParam('special_character_replace')) {
                                    // 按照邮件《关于敏感字符的安全拦截》javascript关键字不需要替换
                ...
                success: function (data, textStatus, xhr) {
                    ...
                    if ((code === 1 || code === 200)) {
                        ...
                        if (typeof epoint.onPreload === 'function') {
                            epoint.onPreload(controls, custom, opts.dtoMethod);
                        }

                        epoint.fire('preload', { controls: controls, custom: custom, method: opts.dtoMethod });
                        ...
                        if (epoint.onBeforeInit) {
                            epoint.onBeforeInit(custom, opts.dtoMethod);
                        }
                        epoint.fire('beforeInit', { custom: custom, method: opts.dtoMethod });

                        opts.done && opts.done.call(that, custom);

                        if (epoint.onAfterInit) {
                            epoint.onAfterInit(custom, opts.dtoMethod);
                        }
                        epoint.fire('afterInit', { custom: custom, method: opts.dtoMethod });
                    } else {
                        //操作失败
                        text = text ? text : '操作失败';
        ...
        validate: function (notShowAlert) {
            ...
            var callback = typeof notShowAlert === 'function' ? notShowAlert : null;

            for (i = 0, len = this.forms.length; i < len; i++) {
                if (!this.forms[i].validate()) {
                    result = false;

                    errors = this.forms[i].getErrors();
                    var target = errors[0].el,
            ...
            if (result && len) {
                for (i = 0; i < len; i++) {
                    if (this.extraValidateControl[i].isDisplay() && !this.extraValidateControl[i].isValid()) {
                        result = false;
                    }
                }
            }
            ...
            if (callback) {
                ...
                if (!result) {
                    if (firstErrorLabel) {
                        msg = firstErrorLabel + (epoint_local.validate_failed || '验证失败') + '：' + firstError.errorText
                    }else {
                        msg = errMsg;
                    }
                }
                callback(result, msg, errors);
            }
