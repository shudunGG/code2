(function(win, $) {
	...
	}).on('click', '.roleconfig-btn', function() {
		addRole();
	...
	function addRole() {
		epoint.openDialog("应用自建角色设置",
				'frame/pages/basic/gateway/app/appinfomanage/approlelist?appGuid='
						+ appGuid, roleCallBack, {
					'width' : 700,
					'height' : 550
				});
	}
	...
	function roleCallBack() {
		epoint.refresh([ 'role-contain' ], '', true);
	}
	...
	document.onkeydown = function(e) {
		...
		if ((e.target || e.srcElement).className.indexOf('mini-') > -1) {
			return;
		}
		if (e.keyCode == 13) {
