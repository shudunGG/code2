...
(function ($, exports) {
	//是否开启oAuth2认证
	if (Config.oAuth2.open) {   //如果启用oauth2认证，重写ajax
		$.extend(exports, {
			// 按照最新传参标准的默认实现
	...
	*/
	...
	$.extend(exports, {
		...
		getUrlParams: function (prop) {
			var url = location.href, key = prop;
		...
		encryptUrlParams: function (url) {
			...
			return baseUrl + '?' + encryptedParams;
		},
		...
		decryptUrlParams: function (url) {
			...
			var params = Util.getUrlParams(url, null);
			return url.substr(0, url.indexOf('?') + 1) + _joinUrlParams(params);
		},
