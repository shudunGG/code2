/*
 * @Author: cds
 * @Date: 2018-09-01 17:03:37 
 * @Last Modified by: liub
 * @Last Modified time: 2019-11-06 09:30:52
 * @Description: '与消息中心的代码有些不同，不能直接复用，因为需要同步到个人信息的页面上的信息'
 */
...
(function (win, $) {
    ...
    function onSwitchClick(e) {
        ...
        var needSave = !$(el).parent().hasClass('detail-span-switch');
        // 子开关无开启的 用于给出提醒
        var noSubSwitchOpened = needSave ? true : false;
        // 同步修改控件值
        $.each(oCtrs.masterSwitchs, function (i, item) {
            item.setValue(value);
        });
        ...
        if(noSubSwitchOpened) {
            MessageSetting.save(true);
        }
        ...
        save: function (isParent) {
            ...
            for(var i = 0,len = data.length; i< len; i++) {
                ...
                if(item.state) {
                    ...
                    if(!isEmpty) {
                        var msgTip = '还未设置通知渠道，请前往【高级设置】选择';
                        if(!isParent) {
                            msgTip = '允许通知后，请开启具体的消息通知渠道';
                        }
                        epoint.alert(msgTip, '系统提醒', function (action) {
                            if (action == 'ok') {
    ...
    // 由于框架引入了重放攻击防御，页面中不能同时发多个请求，只能在initPage后再发请求
    if(epoint) {
        epoint.onAfterInit = function() {
            dataRequest = getData(true);
        };
    } else {
        dataRequest = getData(true);
