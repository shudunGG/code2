...
var $beginBtn = $('#set-btn'), $notEditBtn = $('#skip-btn'), $okBtn = $('#enter-btn'), $pwdBox = $('#pwd-box'), $setBox = $(
		'.pwd-box-step.step-1', $pwdBox), $formBox = $('.pwd-box-step.step-2',
		$pwdBox), $pwdRuleTips = $('.pwd-tips-row', $pwdBox), $oldPwd = $('#oldpwd'), $pwd1 = $('#pwd-new1'), $pwd2 = $('#pwd-new2');
	...
	if (password) {
		Util
				.ajax(
						{
							url : epoint
									.dealRestfulUrl('mypasswordmodifyaction/checkPasswordComplex'),
							data : {
				...
				.done(
						function(data) {
							if (data.success) {
								$pwd1.closest('.pwd-form-row').removeClass(
										'error').addClass('success');
								$pwd1.next().addClass('hidden');
								$pwd1.next().attr('title', '');
								$pwd2.closest('.pwd-form-row').removeClass(
										'error').addClass('success');
...
if (window.isResetPwd) {
	$oldPwd
			.on('blur keyup',
					function() {
						var v = this.value, $this = $(this), $tips = $this
								.next(), $inputRow = $this
...
$pwd1
		.on(
				'blur keyup',
				function() {
					var v = this.value, $this = $(this), $tips = $this.next(), $inputRow = $this
							.closest('.pwd-form-row');
					// $tips.removeClass('hidden');
...
$('input[type="password"]').on(
		'focus',
		function() {
			$(this).next().addClass('hidden').closest('.pwd-form-row')
					.removeClass('error').addClass('active').siblings()
					.removeClass('active');
		}).on('blur', function() {
	$(this).closest('.pwd-form-row').removeClass('active');
...
$okBtn.on(
		'click',
		function() {
			if (!check($oldPwd[0].value, $oldPwd.closest('.pwd-form-row'),
					$oldPwd.next())) {
				return;
			}
			...
			if (!$(this).prop('disabled') && !isInSaving) {
				...
				var data = {
					...
					oldPwd : Util.encryptSM2($oldPwd.val())
				};
				// 无论哪种情况，都要存在原始密码
				// 如果是用户修改密码 则会包含原始密码
				// if (window.isResetPwd) {
...
function checkEnableOkBtn() {
	var allOk = true, $inputRows = $pwdBox.find('.pwd-form-row');
...
function savePwd(pwdData) {
	return Util.ajax({
		...
		data : pwdData
			...
			function(data) {
				if (data.success) {
					epoint.alert(data.msg || '密码修改成功，下次请使用新密码登录', '系统提醒',
							function() {
								// 保存成功 TODO
								if (window.isResetPwd) {
									// 用户修改
...
function check(v, $inputRow, $tips) {
	var oldV = $oldPwd.val(), v1 = $pwd1.val(), v2 = $pwd2.val();
	// 空白判断
	if (/\s/.test(v)) {
		$inputRow.removeClass('success').addClass('error');
		$tips.attr('title', '密码中不能存在空白字符');
		return false;
	}
	...
	if (v1 && v2) {
		// 是否相等
		if (v1 !== v2) {
			$pwd1.closest('.pwd-form-row').addClass('error').removeClass(
					'success');
			$pwd1.next().removeClass('hidden');
			$pwd1.next().attr('title', '两次密码不一致');
			$pwd2.closest('.pwd-form-row').addClass('error').removeClass(
					'success');
			$pwd2.next().removeClass('hidden');
		...
		$pwd1Row.removeClass('error').addClass('success').find('.pwd-info')
				.attr('title', '');
		$pwd2Row.removeClass('error').addClass('success').find('.pwd-info')
				.attr('title', '');
	} else {
		if (v1 && currRuleFn(v1)) {
			$pwd1Row.removeClass('error').addClass('success').find('.pwd-info')
