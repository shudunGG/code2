...
(function (win, $) {
    ...
    function activeByIndex(idx) {
        ...
        activeTab($tabsHead.find('>.tab-item').eq(idx).data('target'));
    }
    ...
    // 设置拼音字段为 控件id字段 来支持通过控件id搜索
    miniSearchInput.setPinyinField('ctrId');
    ...
    var ConfigManager = {
            ...
            $tabsBody.find('>.tab-body').each(function (i, item) {
                ...
                $content.find('.fui-block').each(function (i, block) {
                    ...
                    $block.find('.fui-block-bd .form-row').each(function (j, row) {
                        ...
                        var $ctrEls = $row.find('> .form-control > [class*="mini-"]');
                        var ctrIds = $.map($ctrEls, function (ctr) {
                            return ctr.id || '';
                        }).join(',');
            ...
            $tabsBody.find('.tab-body > .config-content').on(
                ...
                Util.debounce(function () {
                    ...
                    if (!$hightNav.hasClass('active')) {
                        $hightNav.addClass('active').siblings().removeClass('active');
                    }
                }, 50)
            );
            // 新版UI中已默认实现该效果，不需要再自己实现
        ...
        highlight: (function () {
            ...
            return function highlight($target) {
                if ($lastEl) {
                    $lastEl.stop(true).clearQueue('highlight').animate(
                        {
                            opacity: 1,
                            border: 'none'
                        },
        ...
        search: function (kw) {
            ...
            $.each(this._searchArr, function (i, item) {
                if (reg.test(item.name) || (item.ctrId && reg.test(item.ctrId))) {
                    res.push({
                        tabId: item.tabId,
                        id: item.id,
                        name: item.name,
...
(function (win, $) {
    ...
    $('body').on('click', '.form-label', function () {
        ...
        var labelText = $this.data('label') || $this.text();
        var viewType = $this.data('view-type') || 'label';

        if (!ctrId) {
            var ctrEl = $this.next().find('>[class*="mini-"]')[0];
            if (!ctrEl || !ctrEl.id) return;
            ctrId = ctrEl.id;
        }

        $this.data('label', labelText);
        $this.data('ctr-id', ctrId);

        if (viewType == 'label') {
            $this.data('view-type', 'id');
            $this.text(ctrId);
        } else {
            $this.data('view-type', 'label');
            $this.text(labelText);
        }
    });
})(this, this.jQuery);
