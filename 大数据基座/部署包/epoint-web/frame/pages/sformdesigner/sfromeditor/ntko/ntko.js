...
var TANGER_OCX_OBJ = document.all("TANGER_OCX"); //控件对象
if(TANGER_OCX_OBJ){
...
var PPassWord = "epoint**";////保护的密钥epoint**
//保护修改模式
function Protect() {
    TANGER_OCX_OBJ.ActiveDocument.protect(0, 0, PPassWord);
}

function UnProtect() {
    TANGER_OCX_OBJ.ActiveDocument.unprotect(PPassWord);
}
function CheckHasBookmark(bmarkName) {
    //bmarkName = "zhengwen";
    var bMarks = TANGER_OCX_OBJ.ActiveDocument.BookMarks;

    if (bMarks.count == 0) {
        return false;
    }

    var bkmkObj = TANGER_OCX_OBJ.ActiveDocument.BookMarks(bmarkName);
    if (!bkmkObj) {
        return false;
    }
    else
...
//读取书签内容
function GetBookmarkValue(bmarkName) {
    //bmarkName = "zhengwen";
    var bMarks = TANGER_OCX_OBJ.ActiveDocument.BookMarks;
    ...
    var bkmkObj = TANGER_OCX_OBJ.ActiveDocument.BookMarks(bmarkName);
    if (!bkmkObj) {
        alert("Word 模板中不存在名称为：\"" + bmarkName + "\"的书签！");
    }
    alert("书签：" + bmarkName + "=" + TANGER_OCX_OBJ.GetBookmarkValue(bmarkName));
}

//设置书签内容
function SetBookmarkValue(bmarkName, bmarkValue) {
    TANGER_OCX_OBJ.SetBookmarkValue(bmarkName, bmarkValue);
}

//获取书签内容
function GetBookmarkValue(bmarkName) {
    return TANGER_OCX_OBJ.GetBookmarkValue(bmarkName);
}

//强制痕迹保留状态
function EnterRevisionMode(boolevalue) {
    TANGER_OCX_OBJ.EnterRevisionMode(boolevalue);
}
...
function OpenEditOffice(url, newofficetype, username) {
	...
	TANGER_OCX_OBJ.AddDocTypePlugin(".pdf","PDF.NtkoDocument","4.0.0.2","../officeedit/OfficeControl/ntkooledocallx64.cab",51,true);

	}else{
	TANGER_OCX_OBJ.AddDocTypePlugin(".tif","tif.NtkoDocument","4.0.0.2","../officeedit/OfficeControl/ntkooledocall.cab",51,true);
	  	TANGER_OCX_OBJ.AddDocTypePlugin(".pdf","PDF.NtkoDocument","4.0.0.2","../officeedit/OfficeControl/ntkooledocall.cab",51,true);
	}
...
//在当前位置插入书签
function Bookmark_Insert(bmarkName) {//插入书签给书签赋值
    var doc = TANGER_OCX_OBJ.Activedocument;
    var sel = doc.Application.selection;
    doc.Bookmarks.add(bmarkName, sel.range);
}

//下列函数可以将当前编辑的文档插入到URL指定的模板文件的书签位置。可用于稍微复杂的套红
function TaoHongUrl_Bookmark(URL, bmarkName) {
    try {
        TANGER_OCX_OBJ.ActiveDocument.Application.Selection.GoTo(-1, 0, 0, bmarkName);   //有的话将光标定位到书签处
...
function editoffice(url, newofficetype) {
    ...
    if ((typeof (url) != "undefined") && (url != "")) {
        ...
        //   alert(filename);
        try {
            TANGER_OCX_OBJ.BeginOpenFromURL("uploadOfficeFile/" + filename);
        } catch (err) {
        }
        ;
    }
...
//保存office文档
function saveFileToUrl(savrUrl, cmd) {
    var fileName = "1"; // trim(document.all("filetitle").value);
    var result, filedot;
    if (IsFileOpened) {
        ...
        retHTML = TANGER_OCX_OBJ.saveToURL(savrUrl, //提交到的url地址
        ...
        if (getIsIE())
        {
            ...
            switch (TANGER_OCX_OBJ.StatusCode) {
                case 0:
                	if(data && data.custom){
                	    console.log(data);
                		if(data && data.custom && data.custom.IsSuccess && data.custom.IsSuccess=="1"){
                    		TANGER_OCX_OBJ.ActiveDocument.Saved = true;
                    		if(TANGER_OCX_OBJ.FullScreenMode)//全屏是点击保存用alert
                    			alert(data.custom.msg);
                    		else
                    		epoint.alert(data.custom.msg,"提示", function(){
                    		...
                    		epoint.alert("保存失败，" + data.custom.msg);
                    	}
                	}
                    break;
                case 1:
...
//非IE浏览器保存后无法执行同步，需要使用事件实现
function OnComplete2(type,code,retHTML){
	if (!getIsIE())
    {
    	var data ;
    	if(retHTML)
...
function saveFileToUrlNoAlert(savrUrl) {
    ...
    if (IsFileOpened) {
        ...
        //alert("TANGER_OCX_OBJ.StatusCode:" + TANGER_OCX_OBJ.StatusCode);
        //alert(retHTML);
...
function TANGER_OCX_SetDocUser(cuser) {
    ...
    TANGER_OCX_OBJ.WebUserName = cuser;
...
function checkBookmarkIsExist(bmarkName){
    return TANGER_OCX_OBJ.ActiveDocument.BookMarks.Exists(bmarkName);
}
...
function setOneKeyReadHead(obj) {
        if (obj != null && obj != 'undefined') {
            try {
                ...
                if (TANGER_OCX_OBJ.ActiveDocument.BookMarks.Exists(BookMarkName)) {
                    ...
                    var bookmark_zw = TANGER_OCX_OBJ.ActiveDocument.BookMarks(BookMarkName);
                ...
                //huoqu
                //插入模板
                //TANGER_OCX_OBJ.AddTemplateFromURL("http://localhost:8088/EpointOA9/openFawenTemplent.action?cmd=getcontents&templateguid=" + obj);
                //http://localhost:8081/EpointOA9/openFawenTemplent.action?cmd=getcontents&templateguid=8bbfdee8-9d9c-4df3-965b-9d3986af7ad6
                //TANGER_OCX_OBJ.OpenFromURL(Util.getRootPath()+"openFawenTemplent.action?cmd=getcontents&templateguid="	+ obj);
                TANGER_OCX_OBJ.OpenFromURL(Util.getRootPath() + "attachAction.action?cmd=getContent&attachGuid=" + obj);
                //TANGER_OCX_OBJ.OpenFromURL(Util.getRootPath()+"openFawenTemplent.action?cmd=getcontents&templateguid="	+ obj);
                var  a=  TANGER_OCX_OBJ.ActiveDocument.TrackRevisions;
                ...
                if (browser != "firefox") {
                    ...
                    TANGER_OCX_OBJ.ActiveDocument.Bookmarks.Add(
                            BookMarkName, saverange);
                    TANGER_OCX_SetMarkModify(true);
                }
                SetReviewMode(a);//恢复回来的修订状态  这样这个方法不会对文档修订状态造成影响
                AutoUpdate();
            } catch (err) {
                alert("套红失败，请检查模板配置。错误：" + err.number + ":" + err.description);
                openAttach();
            }
            ;
        }
    ...
    function CloseClick() {
            if (!isReadOnly) {
                if (!document.getElementById("TANGER_OCX").ActiveDocument.Saved) {
                    /* epoint.confirm("文档修改过，还没有保存，是否需要保存？", "", function () {
                     saveAndClose();
                     }); */
                    mini.showMessageBox({
                        title: "提醒",
