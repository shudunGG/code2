...
if(oaeditor.clientOs =="Win") {
    DocFrame = document.getElementById("webyozo_id");
...
function attDcodePic(picURL) {
    if (IsFileOpened) {
        ...
        try {
            ...
            //DocFrame.insertPicture(window.location.protocol+"//"+window.location.host+picURL,50,50,30,60);
            DocFrame.insertBarCode(window.location.protocol+"//"+window.location.host+picURL,4,0,150,26,"11.png");
            SetReviewMode(true);//恢复回来的修订状态  这样这个方法不会对文档修订状态造成影响
...
function checkBookmarkIsExist(bmarkName){
    return DocFrame.isExsitBookMark(bmarkName);
}
...
function setOneKeyReadHead(obj) {
    if (obj != null && obj != 'undefined') {
        ...
        if (DocFrame.isExsitBookMark(BookMarkName)) {
            //选择正文内容
            DocFrame.selectBookMark(BookMarkName);
            DocFrame.cut();
        }
        else {
            //选择对象当前文档的所有内容
        ...
        SetReviewMode(false);//修订模式
        DocFrame.cursorToBookMark(BookMarkName);
        DocFrame.paste(1);
...
function TANGER_OCX_EnableReviewBar(boolvalue) {
    if(browser =="IE"){
        DocFrame.CommandBars("Reviewing").Enabled = boolvalue;
        DocFrame.CommandBars("Track Changes").Enabled = boolvalue;
        DocFrame.IsShowToolMenu = boolvalue; //关闭或打开工具菜单
    }else{
        DocFrame.CommandBars.get_Item("Reviewing").Enabled = boolvalue;
        DocFrame.CommandBars.get_Item("Track Changes").Enabled = boolvalue;
        //DocFrame.IsShowToolMenu = boolvalue; //关闭或打开工具菜单
    }
}
...
function TANGER_OCX_SetReviewMode(boolvalue) {
    //进入痕迹保留状态
    DocFrame.setRevisionStatus(boolvalue);
}