<template>
  <div class="design-container">
    <Header ref="header" :in-preview="showPreview" :load="dataLoaded" :formTitle="formTitle" @form-tpl-change="replaceDesign" @save-click="saveToServer()"
      @drop-item-click="handleCommand" />

    <Preview v-if="showPreview" :tableGuid="tableGuid" :designId="designId" />
    ...
    <div class="design-body" v-if="!showPreview" :class="{ narrow: narrowMode, 'view-pc': viewType === 'pc', 'view-mobile': viewType === 'mobile'  }">
      ...
      <div class="design-leftbar">
        <Tabs :value="leftActiveTab" @input="activeLeftTab">
          ...
          <TabPanel label="控件" name="control">
            <PanelControl />
          </TabPanel>
          <TabPanel label="数据域" name="field">
            <PanelShare />
          </TabPanel>
        </Tabs>
        <!-- <AccordionGroup> -->
      ...
      <div class="design-rightbar" :class="{ pickup: narrowMode && rightbarHidden }">
        <el-button v-if="narrowMode && viewType === 'pc'" class="design-rightbar-toggle" icon="el-icon-d-arrow-right"
          plain @click="rightbarHidden = !rightbarHidden"></el-button>
...
export default {
  ...
  methods: {
      ...
      if (data.msg) {
        if (data.fields && data.fields.length) {
          ...
          data.fields.forEach((item) => {
            that.$store.commit('highlightDesignField', {
              id: item.controlId,
    ...
    clearHighlight() {
      ...
      this.$store.commit('clearAllHighlightField');
    },
    clear() {
      this.design = [];
    },
    ...
    saveToServer(noTips) {
      return this.autoCreateField()
        ...
        .then((data) => {
          ...
          return this.toSave(noTips, data);

          // 先校验字段
          //   this.$httpPost(window.formDesignerActions.verificationUrl, data)
          //     .then(resData => {
          //       if (resData.msg) {
          //         this.checkFields(resData);
          //         this.$confirm('是否忽略提示继续保存?', '提示', {
          //           confirmButtonText: '继续保存',
          //           cancelButtonText: '取消',
          //           type: 'warning'
          //         })
          //           .then(() => {
          //             this.toSave(noTips, data);
          //           })
          //           .catch(() => {});
          //       } else {
          //         this.clearHighlight();
          ...
          //       this.$message({
          //         title: '系统提醒',
          //         message: '校验字段失败: ' + err.message,
          //         type: 'error',
          //         showClose: true
          //       });
          //     });

          // return this.$httpPost(window.formDesignerActions.saveUrl, data);
        });
    },
    // 提交保存
    toSave(noTips, data) {
    ...
    autoSave(noTips) {
      ...
      //   return this.$httpPost(window.formDesignerActions.verificationUrl, merge({ designId, tableGuid }, data)).then(
      //     res => {
      //       const custom = res;
      //       if (custom.msg) {
      //         this.checkFields(custom);
      //       } else {
      //         this.clearHighlight();
      //       }
      //       this.$httpPost(window.formDesignerActions.autoSaveUrl, merge({ designId, tableGuid }, data))
      //         .then(res => {
      //           const custom = res;
      //           if (parseInt(custom.msg, 10) == 1) {
      //             !noTips &&
