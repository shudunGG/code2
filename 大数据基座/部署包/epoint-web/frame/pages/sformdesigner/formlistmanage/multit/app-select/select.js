...
(function(win, $) {
	...
	$bottom.on('click', '.prev-btn', function() {
		...
		var designId = Util.getUrlParams('designId');
		if (relationguid == "" || relationguid == "undefined") {
			window.parent.goPrev(rowguid, isSub);
		} else {
			window.parent.goCustomPrev(rowguid, isSub, relationguid,designId);
		}
		...
		Util.ajax({
			...
			if(data.relationguid){
				window.parent.goCustomNext(rowguid, isSub, data.relationguid,designId);
			}else{
