<template>
    <div class="left-panel">
        <Accordion title="基础控件" v-if="baseControls.length">
            <draggable class="control-drag-area" :list="baseControls" :group="{ name: 'control', pull: 'clone', put: false }" swapThreshold="0.3" :sort="false" :clone="createControl" @change="log" :emptyInsertThreshold="20" :move="checkMove">
                <div class="template-item control-tpl-item" v-for="(control, index) in baseControls" :key="control.type + index" :data-control-type="control.type">
        ...
        <Accordion title="扩展控件" v-if="extControls.length">
            <draggable class="control-drag-area" :list="extControls" :group="{ name: 'control', pull: 'clone', put: false }" swapThreshold="0.3" :sortable="false" :clone="createControl" @change="log" :move="checkMove">
                <div class="template-item control-tpl-item" v-for="(control, index) in extControls" :key="control.type + index" :data-control-type="control.type">
                    <span class="template-item-icon control" :class="control.icon"></span>
                    <span class="template-item-name">{{ control.name }}</span>
                </div>
            </draggable>
      ...
      <div class="template-item control-tpl-item" v-for="(control, index) in commonControls" :key="control.type + index" :data-control-type="control.type">
        <span class="template-item-icon control" :class="control.icon"></span>
        <span class="template-item-name">{{ control.name }}</span>
      </div>
    </draggable> -->
        ...
        <div class="template-item control-tpl-item" v-for="control in commonControls" :key="control.type" :data-control-type="control.type">
          ...
          <span class="template-item-name">{{ control.name }}</span>
            ...
            <template v-for="catalog in commonControlsTree">
                ...
                <draggable :key="'body-' + catalog.id" class="control-drag-area" :list="catalog.children" :group="{ name: 'control', pull: 'clone', put: false }" swapThreshold="0.3" :sortable="false" :clone="createdCommonControl" @change="log" :move="checkMove">
                    <div class="template-item control-tpl-item" v-for="control in catalog.children" :key="catalog.id + control.name + control.type" :data-control-type="control.type" :title="control.name">
                        <span class="template-item-icon control" :class="control.icon"></span>
                        <span class="template-item-name">{{ control.name }}</span>
                    </div>
                </draggable>
