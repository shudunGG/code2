<template>
  <div class="col" :id="col.id" :name="col.name" v-if="hideTables.indexOf(col.id) < 0" :class="getColCls(col, index, row.cols.length)" :style="colStyle" @click.stop="handleColClick(index, col, $event)" @contextmenu.stop.prevent="onContextMenu">
    <!-- <slot /> -->
    <!-- 手风琴 -->
    ...
    <!-- <RowLayout :list="col.subs" :level="levelNext" :parentCol="col" :parentRow="row" v-show="type == 'acc-item' || (col.subs && col.subs.length) || inLayoutDragging" v-if="!col.controls.length && row.cols.length != 1"></RowLayout> -->
    ...
    <draggable class="dropArea controls" group="control" :list="col.controls" v-if="showControlContainer" :class="{ active: col.controls.length || inControlDragging }" @start="onControlDragStart" @end="onControlDragEnd" @add="onControlAdd($event, { col, colIndex: index, row })" draggable=".design-control" :swapThreshold="1" :invertedSwapThreshold="1" :invertSwap="false" direction="vertical" :move="checkControlMove">
      ...
      <Control v-for="control in col.controls" :control-data="control" style="display:inline-block;" :key="control.id" :class="{ selected: control.id == selectedControlId }" @click.native.stop="onControlClick(control.id)"></Control>
      <!-- </div> -->
    </draggable>
...
const hideTables = search.get('hideTables') || '';
...
export default {
  ...
  created() {
      this.hideTables = hideTables.split(',');
  },
  computed: {
  ...
  methods: {
    ...
    onControlAdd(ev, data) {
      ...
      if (ev.from.classList.contains('control-drag-area')) {
        ...
        if (isEmpty) {
          const prevName = ev.item.dataset.name || '';
          this.$store.dispatch('autoAddLabel', { parentCol: prevCol, prevName });
        }
      }
    },
    /**
