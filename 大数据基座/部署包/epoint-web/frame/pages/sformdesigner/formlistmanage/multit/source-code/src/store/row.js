...
export default {
  getters: {
    rowDataMap: state => {
      ...
      function addRowData(row, index) {
        // const rowId = (row.id+'').replace(/^col-(\w+)-\d{1,2}$/, '$1');    
        if (map[row.id]) {
  ...
  mutations: {
    ...
    replaceToMulti(state, payload) {
      ...
      const multiRows = createMultiRowCol(row, rowCount, colCount);

      container.splice(rowIndex, 1, ...multiRows);

      function createMultiRowCol(base, r, c) {
        ...
        colBase.style.width = colBase.style.width / c;
