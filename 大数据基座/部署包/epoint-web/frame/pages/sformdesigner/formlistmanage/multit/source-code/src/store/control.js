import { getDotProp, setDotProp, copy, controlAutoIndex } from '../util/index.js';
...
export default {
  getters: {
    ...
    controlNameMap: ({ hiddenControls }, { allControls }) => {
      ...
      hiddenControls.forEach(ctr => {
        map[ctr.id] = ctr.name || `${ctr.namePrefix}_${ctr.autoIndex}`;
      });
      allControls.forEach(ctr => {
        map[ctr.id] = ctr.name || `${ctr.namePrefix}_${ctr.autoIndex}`;
      });
  ...
  actions: {
    ...
    autoAddLabel(ctx, payload) {
      ...
      const control = createLabel(prevName, ctx.state.globalStyle);
 ...
 */
function createLabel(labelText, globalStyle) {
