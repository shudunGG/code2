...
export default {
  ...
  computed: {
    ...mapState(['formSize']),
    rowHeight() {
      const g = this.$store.state.globalStyle;
      return g && g.cellHeight ? g.cellHeight : MIN_ROW_HEIGHT;
    },
    initialBorder() {
  ...
  methods: {
    ...
    checkMove(ev) {
      ...
      if (dragData.cols == 1 && !ev.to.classList.contains('level-0') && !ev.to.parentElement.classList.contains('acc-item')) {
        debugger
