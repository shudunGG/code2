...
export default {
  ...
  created() {
    ...
    this.$bus.$on('controlClick', this.labelTextAutoFocus);
    // delete 删除
    document.addEventListener('keyup', this.deleteShortcutKey);
  ...
  beforeDestroy() {
    this.$bus.$off('controlClick');
    document.removeEventListener('keyup', this.deleteShortcutKey);
  },
  data() {
    return {
      // controlTypeMap,
  ...
  methods: {
    deleteShortcutKey(ev) {
      // 排除掉输入标签 防止误删除
      if (ev.target && ['TEXTAREA', 'INPUT'].indexOf(ev.target.nodeName) !== -1) {
        return;
      }
      // delete
