<template>
  <div class="design-area" id="design-area" ref="designArea" style="position:relative" :data-width="size.width" :data-height="size.height" :style="{ width: size.width + 20 + 'px' }" @click="handleFormClick">
    ...
    <v-contextmenu ref="ctxMenu">
      ...
      <v-contextmenu-item @click="ctxMneuItemHandle('removeCol')" :disabled="!ctxMenuData.colDeletable || ctxMenuData.isControl">{{ ctxMenuData.type == 'acc-layout' ? '删除此手风琴项' : '删除此列'}}</v-contextmenu-item>
      ...
      <v-contextmenu-item @click="ctxMneuItemHandle('removeRow')" :disabled="ctxMenuData.isControl">{{ctxMenuData.type == 'acc-layout' ? '删除此手风琴' : '删除此行'}}</v-contextmenu-item>
      ...
      <v-contextmenu-item divider v-if="ctxMenuData.isCol"></v-contextmenu-item>
      <v-contextmenu-item v-if="ctxMenuData.isCol" @click="handleQuickSplit()" :disabled="ctxMenuData.isControl || ctxMenuData.hasControl">快速拆分</v-contextmenu-item>
    </v-contextmenu>
  ...
  methods: {
    ...
    adjustCtxPos(eventX, eventY) {
      ...
      this.$nextTick(() => {
        ...
        const width = this.$refs.ctxMenu.$el.clientWidth;
        const height = this.$refs.ctxMenu.$el.clientHeight;
        ...
        this.$refs.ctxMenu.$el.style.top = `${pos.top}px`;
        this.$refs.ctxMenu.$el.style.left = `${pos.left}px`;
      });
    },
    showCtxMenu(event, data, handle) {
      ...
      this.ctxMenuData.hasControl = data.hasControl;
      this.ctxMenuData.isCol = !this.ctxMenuData.isControl && this.ctxMenuData.type == 'row-layout';
    ...
    // 右键快速拆分
    handleQuickSplit() {
      this.rowData.rowCount = 2;
      this.rowData.colCount = 2;
      this.showMutliRowConfirm(() => {
        ...
        if (rowCount === 1 && colCount !== 1) {
          this.ctxMneuItemHandle('insertMultiCol', { colCount });
          return;
        }

        this.ctxMneuItemHandle('insertMultiRowCol', { rowCount, colCount });
        return;
      });
    }
  }
