export default {
  methods: {
    onContextmenu(event, data) {
      ...
      if (data.row.cols.length == 1 || (data.row.cols.length === 2 && data.parentCol && data.parentCol.subs.length === 1)) {
        data.colDeletable = false;
      }

      // 是否含有控件
      if (data.col && data.col.controls && data.col.controls.length) {
    ...
    handleContextMenu(type, opt) {
      ...
      switch (type) {
        ...
        case 'insertColRight':
          ...
          break;
        case 'insertMultiCol':
          // 快速拆分 输入的行数目为 1 的情况，则插入列即可, 同向左向右插入
          this.splitCol({
            col: data.col,
            colIndex: data.colIndex,
            count: opt.colCount || 2,
            parentRow: data.row,
            direction: 'right',
            shuttle: true
          });
          break;
        case 'insertMultiRowCol':
          // 快速拆分 输入行大于 1 的情况
          console.log(opt);
          this.$store.dispatch('insertMultiRowCol', {
            col: data.col,
            parentCol: data.parentCol,
            parentRow: data.row,
            parentRowIndex: data.rowIndex,
            rowCount: opt.rowCount,
            colCount: opt.colCount,
            shuttle: true
          });
          break;
