...
export default {
  methods: {
    onMouseDown(ev) {
      ...
      this.startHeight = this.rowData.height || MIN_ROW_HEIGHT;
    ...
    getXLimit(ev, containerRect) {
      ...
      if (!this.colData) {
        ...
        // 还需要保证一个最小宽度
        x1 = x1 < MIN_COL_WIDTH ? 0 : x1 - MIN_COL_WIDTH;
      }
      ...
      if (!this.nextColData) {
        ...
        // 还需要保证一个最小宽度
        x2 = x2 < MIN_COL_WIDTH ? 0 : x2 - MIN_COL_WIDTH;
      }
    ...
    getYLimit(ev, containerRect) {
      ...
      let y1, y2;
      const rowHeight = getHeight(this.rowData) || MIN_ROW_HEIGHT;
      y1 = rowHeight - MIN_ROW_HEIGHT;
      if (y1 < 0) {
        y1 = 0;
      }
