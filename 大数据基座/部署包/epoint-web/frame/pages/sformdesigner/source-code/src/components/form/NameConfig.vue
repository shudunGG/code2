<template>
  <div class="component-name-config config-block">
    <h3 class="component-type config-category">
      {{ type }}
      <span class="remove-component" @click="onComponentRemove">删除</span>
    </h3>
    <div class="component-name-block">
      <div class="component-name" @click="(editorValue = name), (showEditor = true)" v-show="!showEditor" :title="name">{{ name }}</div>
      <div v-if="showEditor">
        <!-- <el-input v-model="editorValue" size="mini" style="width:50%;margin-right:6px;" @keyup.enter="submitName" /> -->
        <div class="el-input el-input--mini" style="width:50%;margin-right:6px;">
          <input class="el-input__inner" v-model="editorValue" size="mini" @keyup.enter="submitName" />
        </div>
