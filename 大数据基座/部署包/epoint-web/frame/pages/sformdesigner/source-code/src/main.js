...
/**
 * 统计分析移动端使用情况 暂时以全局变量直接记录
 *
 * 进入移动设计器则标记 mobileDesignEntered 为 true
 * 移动控件则记录  mobileDesignMoved 为 true
 */
window.FORM_DESIGNER_ANALYSIS = {
  mobileDesignEntered: false, // 移动端是否进进入过
  mobileDesignMoved: false, // 移动端是否移动过
  mobileDesignAdd: false // 是否新增过数据域中的控件
};
Object.defineProperty(window.FORM_DESIGNER_ANALYSIS, 'reset', {
  enumerable: false,
  value: function() {
    this.mobileDesignEntered = false; // 移动端是否进进入过
    this.mobileDesignMoved = false; // 移动端是否移动过
    this.mobileDesignAdd = false; // 是否新增过数据域中的控件
  }
});
