...
var epointsform = {
	 ...
	 *            初始化失败回调事件
	 */
	init : function(formId, divId, formType, param, successCallBack,
			failedCallBack) {
		if (!param) {
			param = {};
		...
		try {
			...
			var versionGuid = "";
			if(param.vguid){
				versionGuid = param.vguid;
			}
			
			// 获取表单地址的iframe
			document.getElementById("epointsformPrintIframe").src = epointsform
					.getRightUrl(
							"frame/pages/sformdesigner/formlistmanage/epointsformversionsearch?formId="
									+ formId + "&formType=" + formType
									+ "&mobilePrefix="
									+ window.epointsform.mobilePrefix
									+ "&versionGuid=" + versionGuid
									+ "&printPageType=" + param.printPageType,
							window.epointsform.epointUrl);
			var customUrl = "epointtemp/epointsform/form/";
			...
			window.epointsform.messengerObject
					.listen(function(msg) {
						...
						if (msg.handleType == "getFormUrl") {
							...
							var formId = msg.formUrl.formId;
							if (msg.formUrl.customUrl != null
									&& msg.formUrl.customUrl != "") {
							...
							if (!detailUrl) {
								...
								if (window.epointsform.printPageType == "1") {
									detailUrl = customUrl + formId + "/"
											+ versionId + "/"
											+ window.epointsform.mobilePrefix
											+ "detail?versionInfo=" + versionId;
								} else {
									detailUrl = customUrl + formId + "/"
											+ versionId + "/"
											+ "printdetail?versionInfo="
											+ versionId;
								}
							}
							...
							// 根据类型拼接Url
							if (!addUrl
									&& ((formType == 'add') || (formType == 'detail'))) {
								addUrl = customUrl + formId + "/" + versionId
										+ "/" + window.epointsform.mobilePrefix
										+ formType + "?versionInfo="
										+ versionId;
							...
							if (msg.error
								...
								/*
								 * document.getElementById("epointsformIframe").src =
								 * epointsform .getNewFormUrl( document
								 * .getElementById("epointsformIframe").src,
								 * msg.rowGuid);
								 * window.epointsform.messengerObject
								 * .addTarget( document
								 * .getElementById("epointsformIframe").contentWindow,
								 * 'epointsformIframe'); document
							...
							if (msg.height && msg.iframeId) {
								$('#fui-content').find('#' + msg.iframeId).css(
										'height', msg.height);
							}
							if (msg.error
									&& window.epointsform.initFailedCallBack) {
							...
							if (msg.error
								...
								window.epointsform.convertFailedCallBack.call(
										this, msg.error);
							} else if (msg.formData
									&& window.epointsform.convertSuccessCallBack) {
								window.epointsform.convertSuccessCallBack.call(
										this, true);
							}
	 ...
	 *            失败验证表单回调事件
	 */
	validate : function(notShowAlert, successCallBack, failedCallBack) {
	 ...
	 * @param savePath
	 *            转换后pdf存放路径
	 * @param convertSuccessCallBack
	 *            表单转Pdf成功回调事件
	 * @param convertFailedCallBack
	 *            表单转Pdf失败回调事件
	 */
	convertHtml2Pdf : function(convertPath, savePath, convertSuccessCallBack,
			convertFailedCallBack) {
	 ...
	 *            表单转Pdf失败回调事件
	 */
	convertHtml2Pic2Pdf : function(convertSuccessCallBack,
			convertFailedCallBack) {
		window.epointsform.convertFailedCallBack = convertFailedCallBack;
		window.epointsform.convertSuccessCallBack = convertSuccessCallBack;
		var msg = {};
	...
	getRootPath : function() {
		var loc = window.location, host = loc.hostname, protocol = loc.protocol, port = loc.port ? (':' + loc.port)
				: '', path = (epointsform._rootPath != undefined ? epointsform._rootPath
