        ...
        $([_text.node, _img.node]) // _name.node,
           .mouseover(function (e) {
               if (_o.style != "BlackAndWhite"&&_o.type != "Begin" && _o.type != "End" && _o.type != "Router") {
                   ...
                   url = Util.addUrlParams("activityinstanceinfobrowser?processVersionInstanceGuid=" + ProcessVersionInstanceGuid + "&activityInstanceGuid=" + Arry[1] + "&mode=" + mode , Util.getUrlParams(), "ignore");
                   if (document.getElementById('PageDetail').src != url)
                       document.getElementById('PageDetail').src = url;
               }
               else if(Util.getUrlParams("mode")=="manage" && _o.style == "BlackAndWhite"&&_o.type != "Begin" && _o.type != "End" && _o.type != "Router"){
            	   var X = e.clientX + document.body.scrollLeft;
                   var Y = e.clientY + document.body.scrollTop;
                   $("#" + DetailPanelId).css({ "top": Y, "left": X }).show();
                   var Arry = _o.detail.split('★');
                   ...
                   var ProcessVersionInstanceGuid=Util.getUrlParams("processVersionInstanceGuid");
                   var mode="unactive";
                   url = Util.addUrlParams("activityinstanceinfobrowser?processVersionInstanceGuid=" + ProcessVersionInstanceGuid + "&activityGuid=" + Arry[2] + "&mode=" + mode, Util.getUrlParams(), "ignore");
                   if (document.getElementById('PageDetail').src != url)
                       document.getElementById('PageDetail').src = url;
               }
           })
