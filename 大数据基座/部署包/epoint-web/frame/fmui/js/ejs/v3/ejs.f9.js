...
// 公共样式
var commonClassName$3 = function commonClassName() {
...
function getOuListWithUser(ouGuid) {
  ...
  Util.ajax({
    ...
    success: function success(result) {
      ...
      if (parseInt(result.status.code) === 1) {
...
function createEditPage() {
  ...
  if (myShadowDom.querySelector("#" + ACTION_EDIT_UNIQUE_ID + " .selected-users")) {
    new Sortable(myShadowDom.querySelector("#" + ACTION_EDIT_UNIQUE_ID + " .selected-users ul"), {
      // eslint-disable-line
      disabled: false,
      delay: 300,
...
function getProjectBasePath(reg) {
  var reg = reg || "/frame/";
  var basePath = "";
  ...
  var pathCompatibles = ["/showcase/", "/pages/", "/epointtemp/", "/"];
...
function selectPerson(params, hybrid, success) {
  ...
  if (!myShadowDom) {
    ...
    // 引入mui样式
    var basePath = getProjectBasePath("/js/");
