    ...
    exports.ajax = function (paramsData, successCallback, errorCallback, options) {
        ejs.runtime.getPlatformUrl({
            success: function (result) {
                var platformUrl = result.platformUrl;
                if (platformUrl.indexOf('mobileappclient') === -1) {
                    platformUrl += '/mobileappclient'
                }
                if (platformUrl.slice(-1) != '/') {
                    platformUrl += '/';
                }
                var headerClientId = {};

                // 匿名访问
                if (paramsData.clientId) {
                    headerClientId = {
                    ...
                    // 非匿名访问
                    platformUrl = platformUrl.replace(/mobileappclient/, 'frame');
                }
                var data = {
                ...
                var ajaxBody = {
                    ...
                    url: /http/.test(paramsData.url) ? paramsData.url : platformUrl + paramsData.url,
                    contentType: 'application/x-www-form-urlencoded',
                    data: data,
                    timeout: 10000,
                    dataPath: paramsData.dataPath,
                    isEncrypt: isEncrypt,
                    headers: headerClientId,
                    success: function (result) {
                        // 考虑到网络不佳情况下，业务是否手动显示加载进度条
                        if (paramsData.isShowWaiting) {
                            ejs.ui.closeWaiting();
