...
(function () {
    ...
    BaiduMap.prototype = {
         ...
         * @param {String} isChange 是否需要转换坐标
         * @param {Function} success 成功回调 (point)
         * @param {Boolean} isOrigin 是否是原始坐标，原始坐标和谷歌坐标有些偏差
         */
        _changeBDPointByGeolocationLocation: function (longitude, latitude, isChange, success, isOrigin) {
            // 如果传过来的是百度坐标，不需要转换（自定义api返回的经纬度）
            if (arguments[2] == '0') {

                // 百度定位，直接返回
                var ggpoint = new BMap.Point(longitude, latitude);

                success && success(ggpoint);
        ...
        _getLocationByEJS: function (success, error) {
            // 通过config全局配置是否开启自定义api获取经纬度
            if (Config.customize.isCustomLocation == '1' && Util.os.android) {
                this._callApiGetLocation(success);
            } else {
                // ejs默认拿到的是谷歌坐标，会转成百度
                ejs.runtime.getGeolocation({
                    success: function (result) {
                        success && success(result.longitude, result.latitude, 1);
                    },
                    error: function (err) {
        ...
        getLocation: function (success, error) {
            ...
            var callback = function (longitude, latitude, isChange) {
                // isChange:是否需要转换坐标
                longitude = longitude || options.longitude;
                latitude = latitude || options.latitude;

                self._changeBDPointByGeolocationLocation(longitude, latitude, isChange, success);
            };
