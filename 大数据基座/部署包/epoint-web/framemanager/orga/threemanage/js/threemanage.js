...
var $threeHeader = $('#three-header'), $threeTablelist = $('#three-tablelist'), $threeLayer = $('#three-layer');
...
$threeHeader
		.on(
				...
				'.input-add',
				function() {
					var $this = $(this), name = $this.data('name'), thisInput = miniCollect[name], ids = thisInput
							.getValue(); // 输入框中的值
					...
					epoint
							.openDialog(
									'人员选择',
									"framemanager/orga/orga/user/selectuser?userGuid="
											+ userGuid,
									function(res) {
										// 关闭时回传到当前输入框
										if (res) {
											var rtnValueArray = res
													.split('_SPLIT_');
											...
											var rtnNamesArray = rtnNames
													.split(';')
											var namesStr = rtnNamesArray
													.join(',')
...
function removeItem(e) {
	...
	// 提交保存
	epoint.execute('deleteUser', '', '[' + role + ',' + userGuid + ']',
			function(data) {
...
function addModule() {
	...
	epoint.openDialog('新增功能',
			'framemanager/orga/threemanage/module/threemanagemoduleadd',
...
function saveBtn() {
	...
	saveExpanded();
	epoint.execute('threemanageaction.save', '', expandedKeys, callback);
...
function resetPassword(type){
	...
	if(type=='0'){
		// 配置管理员
		params = mini.get('system-admin');
	}else if(type=='1'){
		// 安全管理员
		params = mini.get('security-admin');
	}else if(type=='2'){
		// 安全审计员
		params = mini.get('security-auditor');
	}else{
		epoint.showTips('无相关角色', {state : 'warning'});
	}
	if(params){
		var ids = params.getValue();
		ids = ids.replace(',', ';');
		epoint.execute('threemanageaction.resetPassword', '', '[' + type + ',' + ids +']', 
				function(data){
					if(data){
						epoint.showTips(data.msg,{state : 'info'});
					}
