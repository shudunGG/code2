...
$threeHeader
		.on(
																...
																if (data) {
																	if (data.success) {
																		...
																		thisInput
																				.setText(namesStr);
																		epoint
																				.showTips(data.msg, {state : 'success'});
																	} else {
																		epoint
																				.showTips(
																						data.msg,
			...
			var confirmMsg = "";
			if (currentStatus == 0) {
				confirmMsg = "请确认开启涉密模式！";
			} else {
				confirmMsg = "请确认关闭涉密模式！";
			}
			epoint.confirm(confirmMsg, '确认', function() {
				epoint.execute('switchSecretStatus', '', function(data) {
					if (data) {
						if (data.success) {
...
function addModule() {
	...
	epoint.openDialog('新增密级',
			'framemanager/metadata/secret/secretlevel/secretlevel',function(param) {
				if (param != 'close') {
...
function secretTableList() {
	...
	epoint
			.openDialog(
					'涉密表管理',
					'framemanager/metadata/secretmanage/secrettablelist/secrettablelist',
...
function saveBtn() {
	...
	epoint.execute('secretmanageaction.save', '', callback);
