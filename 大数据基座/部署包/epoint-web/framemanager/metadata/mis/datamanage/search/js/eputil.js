...
var eputil = {
	...
	// 前台通用的yesNoModel的数据
	yesNoData: [{id: "1", text: "是"}, {id: "0", text: "否"}],
	...
	loadPhoto: function(imgId, attachGuid, defaultPhoto) {
		...
		if(attachGuid) {
			var src = _rootPath + "/rest/attachAction/getContent?isCommondto=true&attachGuid=" + attachGuid;
			var image = new Image();
			image.src = src;
	...
	initCacheControl: function() {
		...
		$buttonedit.each(function(i) {
			...
			if(buttonedit) {
				_this._cacheControls[id] = {value: buttonedit.getValue(), text: buttonedit.getText()};
			}
		});
		...
		$hidden.each(function(i) {
			...
			if(hidden) {
				_this._cacheControls[id] = {value: hidden.getValue()};
			}
		});
		...
		$combo.each(function(i) {
			...
			if(combo) {
				_this._cacheControls[id] = {value: combo.getValue()};
			}
		});
		...
		$output.each(function(i) {
			...
			if(output) {
				_this._cacheControls[id] = {value: output.getValue()};
			}
		});
		...
		$radiolist.each(function(i) {
			...
			if(radiolist) {
				_this._cacheControls[id] = {value: radiolist.getValue()};
			}
		});
	},
	...
	newCallback : function(result) {
		...
		if (message) {
			if (message == 'success') {
				...
				epoint.alert(message, "", null, "info");
			}
		} else {
			epoint.alert('没有返回值！');
		}
	...
	// 工作流回调
	workflowCallback: function(data) {
		if (data && data.msg) {
	...
	deleteFrontRow: function(uid, gridId) {
        ...
        eputil.afterDeleteFrontRow(row, gridId);
	},
	
	afterDeleteFrontRow: function(row, gridId) {
	...
	renderSimpleAttach: function(datas, sourceId) {
		...
		$source.on("click", "ul>li", function() {
			window.location.href = _rootPath +  "/rest/attachAction/getContent?isCommondto=true&attachGuid=" + $(this).data("guid");
		});
	},
	...
	addAttach: function(data, sourceId, $contain) {
		...
		if (eputil.isImage(attachFileName)) {
			src = _rootPath + "/rest/attachAction/getContent?isCommondto=true&attachGuid=" + attachGuid;
		} else {
		...
		$itemBody.on("click", function() {
			...
			if (eputil.isImage(name)) {
				...
				window.location.href = _rootPath + "/rest/attachAction/getContent?isCommondto=true&attachGuid=" + guid;
	...
	/** 代码项树开始* */
	codeItemTreeSelectCall : function(data) {

	},
	openCodeItemTreeSingle : function(e) {
		eputil.openCodeItemTree("single", e.source);
	},
	openCodeItemTreeMulti : function(e) {
		eputil.openCodeItemTree("multi", e.source);
	},
	openCodeItemTree : function(model, source, e) {
		var sourceData;
		var codeName;
		if(source && source.codename){
			sourceData = source;
			codeName = sourceData.codename;
		}else{
			sourceData = e.source;
			codeName = source;
		}
		...
		if (model == 'multi') {
			url = 'framemanager/metadata/mis/datamanage/search/tree/codeitemtreemulti?codename=' + epoint.encodeUtf(codeName);
			param = {
				width : 450,
				height : 500
			};
		} else {
			url = 'framemanager/metadata/mis/datamanage/search/tree/codeitemtreesingle?codename=' + epoint.encodeUtf(codeName);
			param = {
				width : 450,
				height : 500
			};
			...
			epoint.openTopDialog('选择', url, function(rtn) {
				...
				if (rtn != "close") {
					if (sourceData.type == "buttonedit") {
						...
						sourceData.setText(text);
						sourceData.validate(source);
					}
				}
				eputil.codeItemTreeSelectCall(rtn);
			}, param);
		} else {
			epoint.alert("未知的选择树！");
		}
	...
	/** 部门树开始* */
	frameOuTreeSelectCall : function(data) {

	},
	
	selectSingleOu:function(e){
		eputil.openFrameOuBasicTree(e.source, "single");
	},
	selectMultiOu : function(e){
		eputil.openFrameOuBasicTree(e.source, "multi");
	},
	
	openFrameOuBasicTree : function(source, selectType) {
		var style = null;

		if (selectType.indexOf("single") > -1) {
			style = {
				width : 400,
				height : 500
			};
		...
		if(selectType == "multi"){
			url = 'framemanager/orga/orga/ou/selectouwithcheckbox'
		}
		epoint.openDialog('选择部门',url,
				function(param) {
					if (param) {
						var texts ="";
						var guids ="";
						if(selectType.indexOf("single") > -1){
							...
							if(param.indexOf(":")>-1){
								...
								if(s[0].indexOf(";")>-1){
									var p = s[0].split(";");
									var q = s[1].split(";");
									for(var i=0;i<p.length && p.length>0; i++){
										texts += p[i]+";";
										guids += q[i]+";";
									}
								}else{
						...
						source.setValue(guids);
						source.setText(texts);
	...
	selectUser:function(e){
		eputil.openFrameUserTree(e.source);
	},
	
	openFrameUserTree : function(source) {
    	var url='framemanager/orga/orga/user/selectuser';
		epoint.openDialog('选择用户',url,
				function(param) {
					if (param) {
						var texts = "";
						var guids = "";
						if(param.indexOf("_SPLIT_")>-1){
							s = param.split("_SPLIT_");
							if(s[0].indexOf(";")>-1){
								var p = s[0].split(";");
								var q = s[1].split(";");
								for(var i=0;i<p.length && p.length>0; i++){
	...
	openOfficeDialog : function(guid, title, type, source) {
		...
		if(type == "doc" || type == "ppt" || type == "pdf") {
			...
			previewUrl = _rootPath + "/" + eputil.resPath + "excelviewer/viewer?excelGuid=" + guid;
		} else {
	...
	openImageDialog: function(guid, title, source) {
		...
		var src = "";
		if(guid.length < 50) {
			src = _rootPath + "/rest/attachAction/getContent?isCommondto=true&attachGuid=" + guid;
		} else {
			src = guid;
		}
		//根
