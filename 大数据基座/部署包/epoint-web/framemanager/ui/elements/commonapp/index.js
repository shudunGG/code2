...
(function name(win, $) {
    ...
    // 应用模板
    var APP_TPL = '<div class="qm-item" id="{{#isInstalled}}installed-{{/isInstalled}}{{^isInstalled}}qm-{{/isInstalled}}{{code}}" data-id="{{code}}"    title="{{name}}" data-url="{{url}}" data-hassub="{{hasSub}}" data-opentype="{{openType}}" style="width:{{WIDTH}}" title="{{name}}"><div class="qm-item-inner"><span class="qm-item-icon {{icon}}" style="background:{{iconBackColor}};"></span><span class="qm-item-name {{#mutiLine}}long{{/mutiLine}}">{{{shortName}}}</span></div>{{#hasSub}}<div class="sub-qm hidden" data-count="{{subCount}}"><h2 class="sub-qm-name">{{name}}</h2><div class="sub-qm-list">{{{subQms}}}</div></div>{{/hasSub}}</div>';
    ...
    function renderView(data) {
        ...
        $('.qm-page-arrow', $myQms).css('top', $('.qm-pages-list', $myQms).height()/2 + 'px');
        // 处理分页动画
        ...
        var content_height = $('.qm-pages-list').height(),
            rows = Math.floor(content_height / 114);

        if(content_height + 30 > APPS_HEIGHT) {
            // 放不下, 调整 items 的高度
            var item_h = (APPS_HEIGHT - 30) / rows - 20;
            $('.qm-item').height(item_h);
        }
    }
    ...
    function renderQms(data, isSub) {
        ...
        if (!isSub && isOver) {
            ...
            var pageSize = getSize(PAGE_HEIGHT),
                ...
                page = 0,
                i = 0,
                pageHtml = ['<div class="qm-page-bar"><span class="qm-page-prev qm-page-arrow action-icon icon-left disabled"></span>'];
            ...
            while (page < pages) {
                ...
                pageHtml.push('<span class="qm-page-btn  ' + (page === 0 ? ' active' : '') + '" data-id="' + page + '">' + (page === 0 ? '' : '') + '●</span>');
                page++;
            }
            html.push('</div>');
            pageHtml.push('<span class="qm-page-next qm-page-arrow action-icon icon-right"></span></div>');
