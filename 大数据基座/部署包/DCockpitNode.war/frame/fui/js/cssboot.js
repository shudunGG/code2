...
(function(win) {
    ...
    var SrcBoot = {
        ...
        outputSkin: function(isTheme, themeName, skins, currSkin) {
            ...
            for (; i < l; i++) {
                ...
                linkStr = '<link rel="stylesheet' + (isCurr ? '' : ' alternate') + '" name="' + name + '" data-name="' + skin.name + '" title="' + (skin.cname || skin.name) + '" href="' + this.handleResPath(uiSkinPrefix + skin.name + '/skin.css') + '"/>';
                if (isTheme) {
                    linkStr += '<link rel="stylesheet' + (isCurr ? '' : ' alternate') + '" name="' + name + '" data-name="' + skin.name + '" title="' + (skin.cname || skin.name) + '" href="' + this.handleResPath(themeSkinPrefix + skin.name + '/skin.css') + '"/>';
                }
                // output(linkStr);
                // 当前使用的直接输出 其他皮肤迟延加载
                if (isCurr) {
    ...
    // 字体大小放大效果实现
    var fontSizeRatio = getCookie('_font_size_ratio_') || _font_mode_ratio[_font_mode];
    var setBaseFontSize = function() {
        document.documentElement.style.fontSize = calcFontSize(HtmlBaseFontSize, fontSizeRatio);
        window.addEventListener('DOMContentLoaded', function(){
            document.body.style.fontSize = calcFontSize(BodyBaseFontSzie, fontSizeRatio);
        });
    ...
    var arr = [
        // 操作图标样式
        'frame/fui/js/miniui/themes/action/acticons.css',
        // miniui样式
        ['miniui-skin', 'frame/fui/js/miniui/themes/default/miniui.css'],
        // miniui 字体大小缩放
        '',
        // 公共样式
        ['common-skin', 'frame/fui/css/common.min.css'],
        // 公共样式字体缩放
        '',
        // 操作图标，按钮图标
        ...
        // 风格样式
        'frame/fui/mutableui/layout/' + SrcBoot.getThemeConfig(theme).layout + '.css'
    ];

    // ie8将字体图标替换为png图标
    if (document.all && document.querySelector && !document.addEventListener) {
        // 操作图标，按钮图标
        arr[0] = 'frame/fui/js/miniui/themes/action/actimages.css';
    } else {
        arr[4] = 'frame/fui/css/common.scale.css';
        arr[2] = 'frame/fui/js/miniui/themes/miniui.scale.css';
    ...
    SrcBoot.output(arr);
    // 输出控件皮肤 主界面皮肤
    SrcBoot.outputSkin(isThemePage, theme, SrcBoot.getThemeConfig(theme).skins, skin);
