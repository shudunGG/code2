(function (win, $) {
    ...
    CommonDto.prototype = {
                    ...
                    if (LAYOUT_CONTROL.indexOf(control.type) == -1) {
                        ...
                        switch (field.type) {
                            ...
                            case 'largefileuploader':
                                ...
                                // 修改 by liub 2017-09-27 
                                // 系统参数现在是在jsboot请求中统一返回的，不需要再在数据请求中处理了
                    ...
                    if (item.data) {
                        if (control.uiCls == "mini-datagrid" || control.uiCls == "mini-pagertree" || control.uiCls == "mini-treegrid") {
                            ...
                            if (item.total >= 0) {
                                ...
                                control.setShowTotalCount(false);
                            }
                ...
                if (value !== undefined && control.type != 'treegrid') {
                    ...
                    if (this.fields[id]) {
                        ...
                        if (control.type == 'webeditor') {
                            // value = value.replace(/href="javascript:(\w*)\(([^\)]*)\)"/gi, '');
                        ...
                        // 提交数据时将uuid带回去
                        if (control.type == 'verifycode') {
                            this.fields[id].uuid = control.uuid;
                        }
                    }
                    ...
                    if (this.fields[id]) {
                        ...
                        if (control._getEcConfig) {
                            this.fields[id].__ecconfig = control._getEcConfig();
                        }
        ...
        validate: function () {
            ...
            if (!result && mini.isWindowDisplay()) {
        ...
        _parseTree: function (control) {
            ...
            // 嵌套的数据组织形式
            if (control.nodesField != 'children') {
                $.extend(field, {
                    type: "tree-nested",
        ...
        bindBeforeLoad: function (scope) {
            ...
            var controls = mini.findControls(function (control) {
                if (scope) {
                    ...
                    // if (mini.isAncestor(scope, control.el) && control.within) return true;
                }
            ...
            for (i = 0, len = controls.length; i < len; i++) {
                if (controls[i].type == 'dataexport') {
                ...
                } else if (controls[i].type == 'webuploader' || controls[i].type == 'largefileuploader') {
                    // 3个请求都需要加上额外数据
                    ...
                    controls[i].on('beforeload', function (e) {
                        ...
                        // extraId为控件二次请求时需要一起传回后台的控件id
                        var control = e.sender,
                            condition = ((control.type == 'datagrid' || control.type == 'pagertree') && conditionForm) ? conditionForm.slice(0) : [];
                        if (control.extraId) {
